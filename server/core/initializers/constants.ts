import { randomInt } from '@boomboom/boomboom-core-utils'
import {
  AbuseState,
  AbuseStateType,
...
import { isDevInstance, isTestInstance, isTestOrDevInstance, root } from '@boomboom/boomboom-node-utils'
...
export const LAST_MIGRATION_VERSION = 1079

// ---------------------------------------------------------------------------

export const API_VERSION = 'v1'
export const BOOMBOOM_VERSION: string = readJsonSync(join(root(), 'package.json')).version
...
export const SORTABLE_COLUMNS = {
  ...
  // Don't forget to update boomboom-search-index with the same values
...
export const SCHEDULER_INTERVALS_MS = {
  ...
  CHECK_BOOMBOOM_VERSION: 60000 * 60 * 24, // 1 day
...
export const SERVER_ACTOR_NAME = 'boomboom'
...
export const LANGUAGE_HEADER = 'x-boomboom-language'

export const OTP = {
  HEADER_NAME: 'x-boomboom-otp',
...
export const LOG_FILENAME = 'boomboom.log'
export const AUDIT_LOG_FILENAME = 'boomboom-audit.log'
...
if (process.env.PRODUCTION_CONSTANTS !== 'true') {
  if (isTestOrDevInstance()) {
    ...
    SCHEDULER_INTERVALS_MS.CHECK_BOOMBOOM_VERSION = 2000
  ...
  if (isDevInstance()) {
    ...
    VIDEO_LIVE.CLEANUP_DELAY = getIntEnv('BOOMBOOM_TEST_CONSTANTS_VIDEO_LIVE_CLEANUP_DELAY') ?? 5000
...
function buildVideoMimetypeExt () {
  const data = {
    ...
    // it would be confusing for users because BoomBoom will refuse their file (based on the mimetype)
    'video/ogg': [ '.ogv' ],
    'video/mp4': '.mp4'
  }
  ...
  if (CONFIG.TRANSCODING.ENABLED) {
    if (CONFIG.TRANSCODING.ALLOW_ADDITIONAL_EXTENSIONS) {
      ...
      Object.assign(data, {
        ...
        'video/mov': '.mov', // Windows: https://github.com/Chocobozzz/BoomBoom/issues/6669
        'video/x-m4v': '.m4v',
        'video/m4v': '.m4v',
