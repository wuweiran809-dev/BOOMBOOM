...
import { currentDir } from '@boomboom/boomboom-node-utils'
...
async function executeMigration (actualVersion: number, entity: { version: string, script: string }) {
  ...
  const migrationScript = await import(pathToFileURL(join(currentDir(import.meta.url), 'migrations', migrationScriptName)).href)
