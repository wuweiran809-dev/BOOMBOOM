import { guessAspectRatio, sortBy } from '@boomboom/boomboom-core-utils'
import {
...
import { buildPath, root } from '@boomboom/boomboom-node-utils'
import { TranscriptionEngineName, WhisperBuiltinModelName } from '@boomboom/boomboom-transcription'
...
const CONFIG = {
  ...
  SECRETS: {
    BOOMBOOM: config.get<string>('secrets.boomboom')
  },
  HTTP_TIMEOUTS: {
    REQUEST: parseDurationToMs(config.get<number>('http_timeouts.request'))
  },
  DATABASE: {
    DBNAME: config.has('database.name') ? config.get<string>('database.name') : 'boomboom' + config.get<string>('database.suffix'),
  ...
  BOOMBOOM: {
    CHECK_LATEST_VERSION: {
      ENABLED: config.get<boolean>('boomboom.check_latest_version.enabled'),
      URL: config.get<string>('boomboom.check_latest_version.url')
    }
  },
  WEBADMIN: {
    CONFIGURATION: {
...
function getLocalConfigDir () {
  if (process.env.BOOMBOOM_LOCAL_CONFIG) return process.env.BOOMBOOM_LOCAL_CONFIG
