import { uniqify } from '@boomboom/boomboom-core-utils'
import { getFFmpegVersion } from '@boomboom/boomboom-ffmpeg'
import { VideoRedundancyConfigFilter } from '@boomboom/boomboom-models'
import { isProdInstance } from '@boomboom/boomboom-node-utils'
...
async function checkActivityPubUrls () {
  ...
  if (WEBSERVER.HOST !== parsed.host) {
    ...
    logger.warn(
      'It seems BoomBoom was started (and created some data) with another domain name. ' +
...
async function checkFFmpegVersion () {
  ...
  if (major < 4 || (major === 4 && minor < 1)) {
    logger.warn('Your ffmpeg version (%s) is outdated. BoomBoom supports ffmpeg >= 4.1. Please upgrade ffmpeg.', version)
  }
...
function checkSecretsConfig () {
  if (!CONFIG.SECRETS.BOOMBOOM) {
    throw new Error('secrets.boomboom is missing in config. Generate one using `openssl rand -hex 32`')
  }
}
    ...
    if (CONFIG.SIGNUP.ENABLED && CONFIG.SIGNUP.REQUIRES_APPROVAL) {
      logger.warn(
        'SMTP is not configured but signup approval is enabled: ' +
          'BoomBoom will not be able to send an email to the user upon acceptance/rejection of the registration request'
      )
    }
  ...
  if (isProdInstance()) {
    ...
    for (const key of Object.keys(configStorage)) {
      if (configStorage[key].startsWith('storage/')) {
        logger.warn(
          'Directory of %s should not be in the production directory of BoomBoom. Please check your production configuration file.',
          key
        )
      }
    }
