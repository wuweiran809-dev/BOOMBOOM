import { PluginPackageJSON, PluginType, PluginType_Type } from '@boomboom/boomboom-models'
...
export function isNpmPluginNameValid (value: string) {
  return exists(value) &&
    ...
    (value.startsWith('boomboom-plugin-') || value.startsWith('boomboom-theme-'))
}
...
export function isPluginEngineValid (engine: any) {
  return exists(engine) && exists(engine.boomboom)
}
...
export function isThemeNameValid (name: string) {
  return name && typeof name === 'string' &&
    (isPluginNameValid(name) || name.startsWith('boomboom-core-'))
}
