...
import { ActivityPubActor } from '@boomboom/boomboom-models'
...
export function sanitizeAndCheckActorObject (actor: ActivityPubActor) {
  ...
  return exists(actor) &&
    ...
    // In BoomBoom we use this to attach a video channel to a specific account
...
function normalizeActor (actor: ActivityPubActor) {
  ...
  if (actor.summary && typeof actor.summary === 'string') {
    actor.summary = boomboomTruncate(actor.summary, { length: CONSTRAINTS_FIELDS.USERS.DESCRIPTION.max })
