...
import { spdxToBoomboomLicence } from '@server/helpers/video.js'
import validator from 'validator'
import { CONSTRAINTS_FIELDS, MIMETYPES } from '../../../initializers/constants.js'
import { boomboomTruncate } from '../../core-utils.js'
...
export function sanitizeAndCheckVideoTorrentObject (video: VideoObject) {
  ...
  const fail = (field: string) => {
    logger.debug(`Video field is not valid to BoomBoom: ${field}`, { video })
    return false
  }
...
function setRemoteVideoContent (video: VideoObject) {
  if (video.content) {
    video.content = boomboomTruncate(video.content, { length: CONSTRAINTS_FIELDS.VIDEOS.DESCRIPTION.max })
  }
...
function setValidLicence (video: VideoObject) {
  ...
  const spdx = spdxToBoomboomLicence(video.licence.identifier)
