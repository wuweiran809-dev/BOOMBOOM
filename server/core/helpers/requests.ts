...
import { ACTIVITY_PUB, BINARY_CONTENT_TYPES, BOOMBOOM_VERSION, REQUEST_TIMEOUTS, WEBSERVER } from '../initializers/constants.js'
...
export interface BoomBoomRequestError extends Error {
  statusCode?: number
...
export type BoomBoomRequestOptions = {
  timeout?: number
  activityPub?: boolean
...
export const unsafeSSRFGot = got.extend({
  ...
  hooks: {
    beforeRequest: [
      ...
      async options => {
        const httpSignatureOptions = options.context?.httpSignature as BoomBoomRequestOptions['httpSignature']
...
export const boomboomGot = CONFIG.FEDERATION.PREVENT_SSRF
  ? got.extend(gotSsrf, unsafeSSRFGot)
  : unsafeSSRFGot

// ---------------------------------------------------------------------------

export function doRequest (url: string, options: BoomBoomRequestOptions & { preventSSRF?: false } = {}) {
...
export function doJSONRequest<T> (url: string, options: BoomBoomRequestOptions & { preventSSRF?: false } = {}) {
...
export async function doRequestAndSaveToFile (url: string, destPath: string, options: BoomBoomRequestOptions = {}) {
  ...
  try {
    await pipelinePromise(
      boomboomGot.stream(url, gotOptions),
      outFile
    )
  } catch (err) {
    remove(destPath)
...
export function generateRequestStream (url: string, options: BoomBoomRequestOptions = {}) {
  const gotOptions = buildGotOptions({ ...options, timeout: options.timeout ?? REQUEST_TIMEOUTS.DEFAULT })

  return boomboomGot.stream(url, gotOptions)
}
...
export function buildRequestError (error: RequestError) {
  ...
  const newError: BoomBoomRequestError = new Error(error.message)
...
function getUserAgent () {
  return `BoomBoom/${BOOMBOOM_VERSION} (+${WEBSERVER.URL})`
}

function buildGotOptions (options: BoomBoomRequestOptions): OptionsOfUnknownResponseBodyWrapped {
