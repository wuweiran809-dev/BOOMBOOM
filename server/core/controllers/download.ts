import { forceNumber, maxBy } from '@boomboom/boomboom-core-utils'
import { FileStorage, HttpStatusCode, VideoResolution, VideoStreamingPlaylistType } from '@boomboom/boomboom-models'
...
async function downloadGeneratedVideoFile (req: express.Request, res: express.Response) {
  ...
  if (videoFiles.filter(f => f.hasVideo()).length > 1 || videoFiles.filter(f => f.hasAudio()).length > 1) {
    return res.fail({
      ...
      message: `Cannot generate a container with multiple video/audio files. BoomBoom supports a maximum of 1 audio and 1 video file`
    })
  }
