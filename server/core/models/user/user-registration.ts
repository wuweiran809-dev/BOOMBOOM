...
import { UserRegistration, UserRegistrationState, type UserRegistrationStateType } from '@boomboom/boomboom-models'
import {
...
import { cryptPassword } from '@server/helpers/boomboom-crypto.js'
...
export class UserRegistrationModel extends SequelizeModel<UserRegistrationModel> {
  ...
  static getStats () {
    const query = `SELECT ` +
      ...
      `AS "avgResponseTime", ` +
      // "processedAt" has been introduced in BoomBoom 6.1 so also check the abuse state to check processed abuses
