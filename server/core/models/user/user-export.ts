import { FileStorage, UserExportState, type FileStorageType, type UserExport, type UserExportStateType } from '@boomboom/boomboom-models'
...
export class UserExportModel extends SequelizeModel<UserExportModel> {
  ...
  isJWTValid (jwtToken: string) {
    try {
      const payload = jwt.verify(jwtToken, CONFIG.SECRETS.BOOMBOOM, {
        audience: this.filename,
        issuer: WEBSERVER.URL
      })
