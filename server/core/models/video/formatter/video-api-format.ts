import { getResolutionLabel } from '@boomboom/boomboom-core-utils'
import {
  Video,
...
export function videoModelToFormattedJSON (video: MVideoFormattable, options: VideoFormattingJSONOptions = {}): Video {
  const span = tracer.startSpan('boomboom.VideoModel.toFormattedJSON')
...
export function videoModelToFormattedDetailsJSON (video: MVideoFormattableDetails): VideoDetails {
  const span = tracer.startSpan('boomboom.VideoModel.toFormattedDetailsJSON')
