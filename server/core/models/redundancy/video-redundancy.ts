...
import { isTestInstance } from '@boomboom/boomboom-node-utils'
...
export class VideoRedundancyModel extends SequelizeModel<VideoRedundancyModel> {
  ...
  static async findMostViewToDuplicate (randomizedFactor: number) {
    const boomboomActor = await getServerActor()
    ...
    const query = {
      ...
      where: {
        ...
        ...this.buildVideoIdsForDuplication(boomboomActor)
      },
      include: [
  ...
  private static buildVideoIdsForDuplication (boomboomActor: MActor) {
    const notIn = literal(
      '(' +
        ...
        `WHERE "videoRedundancy"."actorId" = ${boomboomActor.id} ` +
        ')'
    )
