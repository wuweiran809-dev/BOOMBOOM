import { UserAdminFlagType, UserRoleType } from '@boomboom/boomboom-models'
...
interface RegisterServerAuthBase {
  ...
  // Called by BoomBoom when a user from your plugin logged out
  // Returns a redirectUrl sent to the client or nothing
  onLogout?(user: MUser, req: express.Request): Promise<string>

  // Your plugin can hook BoomBoom access/refresh token validity
...
export interface RegisterServerAuthPassOptions extends RegisterServerAuthBase {
  // Weight of this authentication so BoomBoom tries the auth methods in DESC weight order
  getWeight(): number

  // Used by BoomBoom to login a user
