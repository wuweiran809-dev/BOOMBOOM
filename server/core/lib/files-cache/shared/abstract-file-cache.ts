...
import { doRequestAndSaveToFile, BoomBoomRequestError } from '@server/helpers/requests.js'
  ...
  private async lazyLoadIfNeeded (filename: string) {
    ...
    if (!file.isLocal() && file.cached === false) {
      ...
      try {
        ...
        if ((err as BoomBoomRequestError).statusCode === HttpStatusCode.NOT_FOUND_404) {
