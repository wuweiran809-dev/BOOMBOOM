import { ffprobePromise, getAudioStream, getVideoStreamDimensionsInfo, getVideoStreamFPS } from '@boomboom/boomboom-ffmpeg'
import { VideoFileStream, VideoLiveEndingPayload, VideoState } from '@boomboom/boomboom-models'
import { boomboomTruncate } from '@server/helpers/core-utils.js'
...
export async function processVideoLiveEnding (job: Job) {
  ...
  // Introduced in BoomBoom 7.2, allow to use the appropriate base directory even if the live privacy changed
...
async function saveReplayToExternalVideo (options: {
  ...
  const truncatedVideoName = boomboomTruncate(liveVideo.name, {
