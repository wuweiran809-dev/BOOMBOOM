import { guessAspectRatio } from '@boomboom/boomboom-core-utils'
import { ActivityIconObject, HttpStatusCode, PlaylistObject } from '@boomboom/boomboom-models'
...
async function buildElementsDBAttributes (elementUrls: string[], playlist: MVideoPlaylist) {
  ...
  await Bluebird.map(elementUrls, async elementUrl => {
    try {
      ...
      const logLevel = (err as BoomBoomRequestError).statusCode === HttpStatusCode.UNAUTHORIZED_401
        ? 'debug'
        : 'warn'
