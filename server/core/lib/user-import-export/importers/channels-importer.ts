...
import { ActorImageType, ChannelExportJSON } from '@boomboom/boomboom-models'
...
export class ChannelsImporter extends AbstractUserImporter<ChannelExportJSON, ChannelExportJSON['channels'][0], SanitizedObject> {
  ...
  protected async importObject (channelImportData: SanitizedObject) {
    ...
    if (existingChannel) {
      logger.info(`Do not import channel ${existingChannel.name} that already exists on this BoomBoom instance`, lTags())
    } else {
