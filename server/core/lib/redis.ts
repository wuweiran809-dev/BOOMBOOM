...
class Redis {
  ...
  static getRedisClientOptions (name?: string, options: RedisOptions = {}, logOptions = false): RedisOptions {
    const connectionName = [ 'BoomBoom', name ].join('')
    const connectTimeout = 20000 // Could be slow since node use sync call to compile BoomBoom
  ...
  private generateContactFormKey (ip: string) {
    return 'contact-form-' + sha256(CONFIG.SECRETS.BOOMBOOM + '-' + ip)
  }
