import { UserNotificationType } from '@boomboom/boomboom-models'
...
export class ChannelChangeOwnershipAccepted extends AbstractChannelChangeOwnershipResponse {
  log () {
    logger.info(
      'Notifying source channel members that ownership change of channel %s was accepted.',
      this.payload.VideoChannel.Actor.preferredUsername
    )
  }

  protected getNotificationType () {
    return UserNotificationType.CHANNEL_OWNERSHIP_CHANGED_ACCEPTED
  }
  ...
  createEmail (user: MUserWithNotificationSetting) {
    ...
    return {
      to,
      subject: t('A channel ownership change request has been accepted', language),
      text: t('{nextOwner} accepted the ownership change request for channel {channelName}.', language, {
