...
import { BoomBoomSocket } from '@server/lib/boomboom-socket.js'
...
export abstract class AbstractJobHandler<C, U extends RunnerJobUpdatePayload, S extends RunnerJobSuccessPayload> {
  ...
  protected async createRunnerJob (
    ...
    if (runnerJob.state === RunnerJobState.PENDING) {
      BoomBoomSocket.Instance.sendAvailableJobsPingToRunners()
    }
  ...
  async complete (options: {
    ...
    if (affectedCount !== 0) BoomBoomSocket.Instance.sendAvailableJobsPingToRunners()
  }
