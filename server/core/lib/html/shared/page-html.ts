import { AVAILABLE_LOCALES, buildFileLocale, escapeHTML, getDefaultLocale, is18nLocale } from '@boomboom/boomboom-core-utils'
import { HTMLServerConfig } from '@boomboom/boomboom-models'
import { isTestOrDevInstance, root, sha256 } from '@boomboom/boomboom-node-utils'
...
export class PageHtml {
  ...
  static addServerConfig (htmlStringPage: string, serverConfig: HTMLServerConfig) {
    ...
    const configScriptTag = `<script type="application/javascript">window.BoomBoomServerConfig = ${serverConfigString}</script>`
