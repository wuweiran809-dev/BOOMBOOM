...
import { EmailPayload, MailAction, SendEmailDefaultOptions, To, UserExportState, UserRegistrationState } from '@boomboom/boomboom-models'
import { getFilenameWithoutExt, isTestOrDevInstance, root } from '@boomboom/boomboom-node-utils'
...
export class Emailer {
  ...
  init () {
    ...
    if (!isEmailEnabled()) {
      if (!isTestOrDevInstance()) {
        logger.error('Cannot use SMTP server because of lack of configuration. BoomBoom will not be able to send mails!')
      }
  ...
  private buildEmailTheme () {
    ...
    // Use default colors, because the admin may have used custom color on a non-boomboom-core theme, which would make the email unreadable
    if (CONFIG.THEME.DEFAULT !== 'boomboom-core-dark-brown' && CONFIG.THEME.DEFAULT !== 'boomboom-core-light-beige') {
      return defaultColorsLight
    }

    const defaultColors = CONFIG.THEME.DEFAULT === 'boomboom-core-dark-brown'
      ? defaultColorsDark
      : defaultColorsLight
