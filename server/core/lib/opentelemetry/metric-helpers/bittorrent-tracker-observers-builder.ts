...
export class BittorrentTrackerObserversBuilder {
  ...
  buildObservers () {
    const activeInfohashes = this.meter.createObservableGauge('boomboom_bittorrent_tracker_active_infohashes_total', {
      description: 'Total active infohashes in the BoomBoom BitTorrent Tracker'
    })
    const inactiveInfohashes = this.meter.createObservableGauge('boomboom_bittorrent_tracker_inactive_infohashes_total', {
      description: 'Total inactive infohashes in the BoomBoom BitTorrent Tracker'
    })
    const peers = this.meter.createObservableGauge('boomboom_bittorrent_tracker_peers_total', {
      description: 'Total peers in the BoomBoom BitTorrent Tracker'
    })
