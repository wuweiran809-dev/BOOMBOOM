...
export class RunnerObserversBuilder {
  ...
  buildObservers () {
    const runnerCount = this.meter.createObservableGauge('boomboom_runner_count', {
      description: 'Number of registered BoomBoom runners'
    })

    const runnerInfo = this.meter.createObservableGauge('boomboom_runner_info', {
      description: 'BoomBoom runner metadata'
    })

    const secondsSinceLastContact = this.meter.createObservableGauge('boomboom_runner_seconds_since_last_contact', {
      description: 'Seconds since the BoomBoom runner last checked in',
      unit: 's'
    })
