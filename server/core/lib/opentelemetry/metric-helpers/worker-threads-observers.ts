...
export class WorkerThreadsObserversBuilder {
  ...
  buildObservers () {
    this.meter.createObservableGauge('boomboom_worker_thread_queue_total', {
      description: 'Total tasks waiting for a BoomBoom worker thread'
    ...
    this.meter.createObservableGauge('boomboom_worker_thread_completed_total', {
      description: 'Total tasks completed in BoomBoom worker threads'
    }).addCallback(observableResult => {
      const stats = getWorkersStats()
