import { getCompleteLocale, getHookType, internalRunHook } from '@boomboom/boomboom-core-utils'
import {
  ClientScriptJSON,
  PluginPackageJSON,
  PluginTranslation,
...
const UNSECURE_PLUGINS_TO_REMOVE = [
  'boomboom-plugin-google-analytics-js'
]
...
export class PluginManager implements ServerHook {
  ...
  registerWebSocketRouter () {
    this.server.on('upgrade', (request, socket, head) => {
      ...
      // Other handlers in BoomBoom will catch this upgrade event too (socket.io, tracker etc)
  ...
  async install (options: {
    ...
    try {
      ...
      ;[ plugin ] = await PluginModel.upsert({
        ...
        enabled: true,
        uninstalled: false,
        boomboomEngine: packageJSON.engine.boomboom
      }, { returning: true })
  ...
  async update (toUpdate: string, fromDisk = false) {
    ...
    // Use the latest version from DB, to not upgrade to a version that does not support our BoomBoom version
    let version: string
    if (!fromDisk) {
  ...
  private async registerPluginOrTheme (plugin: PluginModel) {
    ...
    this.registeredPlugins[npmName] = {
      ...
      boomboomEngine: plugin.boomboomEngine,
      path: pluginPath,
  ...
  private async registerPlugin (plugin: PluginModel, pluginPath: string, packageJSON: PluginPackageJSON) {
    ...
    await ensureDir(registerOptions.boomboomHelpers.plugin.getDataDirectoryPath())
