import { pick, wait } from '@boomboom/boomboom-core-utils'
import {
  ffprobePromise,
...
import { LiveVideoError, LiveVideoErrorType, VideoResolution, VideoState } from '@boomboom/boomboom-models'
...
class LiveManager {
  ...
  private async handleSession (options: {
    ...
    if (oldStreamingPlaylist) {
      ...
      BoomBoomSocket.Instance.sendVideoForceEnd(video)
  ...
  private async onAfterMuxingCleanup (options: {
    ...
    try {
      ...
      BoomBoomSocket.Instance.sendVideoLiveNewState(fullVideo)
