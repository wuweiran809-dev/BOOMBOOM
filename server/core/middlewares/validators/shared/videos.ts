import { HttpStatusCode, ServerErrorCode, UserRight, UserRightType, VideoPrivacy } from '@boomboom/boomboom-models'
...
async function checkCanSeePasswordProtectedVideo (options: {
  ...
  const videoPassword = req.header('x-boomboom-video-password')
...
export async function checkUserQuota (options: {
  ...
  const { channelUser, uploadSize, req, res } = options

  if (await isUserQuotaValid({ channelUserId: channelUser.id, uploadSize }) === false) {
    res.fail({
      status: HttpStatusCode.PAYLOAD_TOO_LARGE_413,
      message: req.t('The user video quota is exceeded'),
