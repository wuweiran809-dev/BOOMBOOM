...
import { ActivityDelete, ActivityPubSignature, HttpStatusCode } from '@boomboom/boomboom-models'
import { isActorDeleteActivityValid } from '@server/helpers/custom-validators/activitypub/actor.js'
import { isHTTPSignatureVerified, parseHTTPSignature } from '@server/helpers/boomboom-crypto.js'
...
async function checkHttpSignature (req: Request, res: Response) {
  return wrapWithSpanAndContext('boomboom.activitypub.checkHTTPSignature', async () => {
...
async function checkJsonLDSignature (req: Request, res: Response) {
  ...
  const { compactJSONLDAndCheckSignature } = await import('../helpers/boomboom-jsonld.js')

  return wrapWithSpanAndContext('boomboom.activitypub.JSONLDSignature', async () => {
