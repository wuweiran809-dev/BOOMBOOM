...
app.use((_req, res, next) => {
  ...
  if (CONFIG.SECURITY.POWERED_BY_HEADER.ENABLED === true) {
    res.setHeader('x-powered-by', 'BoomBoom')
  }
...
import { isHTTPSignatureDigestValid } from './core/helpers/boomboom-crypto.js'
...
import { BoomBoomSocket } from './core/lib/boomboom-socket.js'
...
import { BoomBoomVersionCheckScheduler } from './core/lib/schedulers/boomboom-version-check-scheduler.js'
...
cli
  .option('--no-client', 'Start BoomBoom without client interface')
  .option('--no-plugins', 'Start BoomBoom without plugins/themes enabled')
...
async function startApplication () {
  ...
  BoomBoomVersionCheckScheduler.Instance.enable()
  ...
  // Before BoomBoomSocket init
  PluginManager.Instance.registerWebSocketRouter()

  BoomBoomSocket.Instance.init(server)
