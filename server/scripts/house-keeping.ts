...
import { askConfirmation, displayBoomBoomMustBeStoppedWarning } from './shared/common.js'
...
async function run () {
  ...
  displayBoomBoomMustBeStoppedWarning()
...
async function deleteRemoteFiles () {
  ...
  const res = await askConfirmation(
    `${thumbnails.length.toLocaleString()} thumbnails, ` +
      ...
      `BoomBoom will download them again on-demand. ` +
  ...
  console.log('All remote files deleted! Please restart BoomBoom.')
}
