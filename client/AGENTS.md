...
BoomBoom's web client is an Angular single-page application served
...
- **src/standalone/** — Independently built artifacts:
  - `player/` — BoomBoom video player (Vite build, HLS.js + P2P)
  - `embed-player-api/` — Public npm package for programmatic
    embed control (`@boomboom/embed-api`)
...
- Merge tool: `@boomboom/xliffmerge` (config: `.xliffmerge.json`)
...
- Shared API types come from `@boomboom/boomboom-models` and
  `@boomboom/boomboom-core-utils` (workspace packages)
