...
const debugLogger = debug('boomboom:videos:VideosListComponent')
...
export class VideosListComponent implements OnInit, OnDestroy {
  ...
  onNearOfBottom () {
    ...
    if (window.location.pathname === '/') {
      this.boomboomRouter.silentNavigate([], this.route.snapshot.queryParams)
    }
  ...
  private updateUrl () {
    ...
    if (Object.keys(queryParams).length !== 0 || this.filters.hasBeenCustomizedByUser()) {
      this.boomboomRouter.silentNavigate([], queryParams)
    }
  }
