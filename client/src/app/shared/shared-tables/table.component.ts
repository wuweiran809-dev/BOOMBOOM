...
import { LocalStorageService, Notifier, BoomBoomRouterService, RestPagination, ScreenService } from '@app/core'
import { NgbDropdownModule, NgbTooltipModule } from '@ng-bootstrap/ng-bootstrap'
import { ResultList } from '@boomboom/boomboom-models'
...
import { BoomboomCheckboxComponent } from '../shared-forms/boomboom-checkbox.component'
...
const debugLogger = debug('boomboom:table')
...
> implements OnInit, OnDestroy, OnChanges {
  private boomboomLocalStorage = inject(LocalStorageService)
  private route = inject(ActivatedRoute)
  private boomboomRouter = inject(BoomBoomRouterService)
  ...
  saveSelectedColumns () {
    ...
    this.boomboomLocalStorage.setItem(this.getColumnLocalStorageKey(), JSON.stringify(enabled))
  }

  private loadSelectedColumns () {
    const enabledString = this.boomboomLocalStorage.getItem(this.getColumnLocalStorageKey())
  ...
  private loadTableSettings () {
    try {
      const sort = boomboomLocalStorage.getItem(this.getSortLocalStorageKey())
      if (sort) this.sort = JSON.parse(sort)

      const count = boomboomLocalStorage.getItem(this.getCountLocalStorageKey())
  ...
  private saveTableSettings () {
    if (this.sort.field !== 'match') {
      boomboomLocalStorage.setItem(this.getSortLocalStorageKey(), JSON.stringify(this.sort))
    }

    boomboomLocalStorage.setItem(this.getCountLocalStorageKey(), JSON.stringify(this.pagination.count))
  }
  ...
  private updateUrl (options: {
    ...
    this.boomboomRouter.silentNavigate([ '.' ], newParams, this.route)
  }
