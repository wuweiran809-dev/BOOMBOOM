...
import { ResultList, VideoImport, VideoImportCreate } from '@boomboom/boomboom-models'
...
export class VideoImportService {
  ...
  private extractVideoImports (result: ResultList<VideoImport>): Observable<ResultList<VideoImport>> {
    return this.serverService.getServerLocale()
      .pipe(
        map(translations => {
          result.data.forEach(d => d.state.label = boomboomTranslate(d.state.label, translations))
