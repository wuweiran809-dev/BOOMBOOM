...
import { arrayify, buildDownloadFilesUrl, exists } from '@boomboom/boomboom-core-utils'
import {
  BooleanBothQuery,
  FeedFormat,
  FeedFormatType,
            ...
            .pipe(
              catchError(err => {
                if (err.error?.code === ServerErrorCode.VIDEO_ALREADY_BEING_TRANSCODED && !forceTranscoding) {
                  const message = $localize`BoomBoom considers video "${video.name}" is already being transcoded.` +
                    // eslint-disable-next-line max-len
                    $localize` If you think BoomBoom is wrong (video in broken state after a crash etc.), you can force transcoding on this video.` +
