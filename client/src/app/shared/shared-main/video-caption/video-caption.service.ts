...
import { boomboomTranslate, sortBy } from '@boomboom/boomboom-core-utils'
import { BoomBoomProblemDocument, ResultList, ServerErrorCode, Video, VideoCaption, VideoCaptionGenerate } from '@boomboom/boomboom-models'
      ...
      .pipe(
        ...
        map(({ captionsResult, translations }) => {
          for (const c of captionsResult.data) {
            c.language.label = boomboomTranslate(c.language.label, translations)
          }
  ...
  private generateCaptionOfSpecificVideo (options: {
    ...
    return this.authHttp.post(`${VideoService.BASE_VIDEO_URL}/${video.id}/captions/generate`, body)
      .pipe(
        ...
        catchError(err => {
          const error: BoomBoomProblemDocument = err.error

          if (error?.code === ServerErrorCode.VIDEO_ALREADY_BEING_TRANSCRIBED && !forceTranscription) {
            const message = $localize`BoomBoom considers video "${video.name}" is already being transcripted.` +
              // eslint-disable-next-line max-len
              $localize` If you think BoomBoom is wrong (video in broken state after a crash etc.), you can force transcription on this video.` +
