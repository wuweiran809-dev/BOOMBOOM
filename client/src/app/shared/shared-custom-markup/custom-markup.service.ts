...
} from './boomboom-custom-tags'
import { CustomMarkupComponent } from './boomboom-custom-tags/shared'
...
export class CustomMarkupService {
  ...
  private angularBuilders: { [selector: string]: AngularBuilderFunction } = {
    'boomboom-instance-banner': el => this.instanceBannerBuilder(el),
    'boomboom-instance-avatar': el => this.instanceAvatarBuilder(el),
    'boomboom-button': el => this.buttonBuilder(el),
    'boomboom-video-embed': el => this.embedBuilder(el, 'video'),
    'boomboom-playlist-embed': el => this.embedBuilder(el, 'playlist'),
    'boomboom-video-miniature': el => this.videoMiniatureBuilder(el),
    'boomboom-playlist-miniature': el => this.playlistMiniatureBuilder(el),
    'boomboom-channel-miniature': el => this.channelMiniatureBuilder(el),
    'boomboom-videos-list': el => this.videosListBuilder(el)
  }

  private htmlBuilders: { [selector: string]: HTMLBuilderFunction } = {
    'boomboom-container': el => this.containerBuilder(el)
  }
  ...
  private embedBuilder (el: HTMLElement, type: 'video' | 'playlist') {
    ...
    this.dynamicElementService.setModel(component, {
      ...
      boomboomLink: this.buildBoolean(data.boomboomLink),
  ...
  private containerBuilder (el: HTMLElement) {
    ...
    root.classList.add('boomboom-container', layoutClass)
