import { exists } from '@boomboom/boomboom-core-utils'
import { CustomConfig, VideoCommentPolicy, VideoPrivacy } from '@boomboom/boomboom-models'
...
export class UsageType {
  ...
  private computeRegistration () {
    ...
    if (this.registration === 'open') {
      this.addExplanation($localize`:bullet point of "BoomBoom will\:":<strong>Allow</strong> any user <strong>to register</strong>`)
      ...
      this.addExplanation(
        $localize`:bullet point of "BoomBoom will\:":Allow users to <strong>apply for registration</strong> on your platform`
      )
      ...
      this.addExplanation($localize`:bullet point of "BoomBoom will\:":<strong>Disable</strong> user <strong>registration</strong>`)
    ...
    if (this.registration === 'approval' || this.registration === 'open') {
      this.addExplanation(
        $localize`:bullet point of "BoomBoom will\:":Require <strong>moderator approval</strong> for videos published by your community`
      )
  ...
  private computeVideoQuota () {
    ...
    if (this.videoQuota === 0) {
      this.addExplanation(
        ...
        $localize`:bullet point of "BoomBoom will\:":<strong>Prevent</strong> new users <strong>from uploading videos</strong> (can be changed by moderators)`
      )
    } else if (this.videoQuota === -1) {
      this.addExplanation(
        $localize`:bullet point of "BoomBoom will\:":<strong>Not limit the amount of videos</strong> new users can upload`
      )
    } else {
      this.addExplanation(
        $localize`:bullet point of "BoomBoom will\:":Set <strong>video quota to ${
  ...
  private computeVideoImport () {
    ...
    if (this.remoteImport === 'enabled') {
      this.addExplanation(
        // eslint-disable-next-line max-len
        $localize`:bullet point of "BoomBoom will\:":<strong>Allow</strong> your users <strong>to import and synchronize</strong> videos from remote platforms (YouTube, Vimeo...)`
      )
    } else {
      this.addExplanation(
        // eslint-disable-next-line max-len
        $localize`:bullet point of "BoomBoom will\:":<strong>Prevent</strong> your users <strong>from importing videos</strong> from remote platforms`
      )
    }
  }
  ...
  private computeStreamLives () {
    ...
    if (this.live === 'enabled') {
      this.plugins.push('boomboom-plugin-livechat')

      this.addExplanation(
        // eslint-disable-next-line max-len
        $localize`:bullet point of "BoomBoom will\:":<strong>Allow</strong> your users <strong>to stream lives</strong> and chat with their viewers using the <strong>Livechat</strong> plugin`
      )
    } else {
      this.addExplanation(
        $localize`:bullet point of "BoomBoom will\:":<strong>Prevent</strong> your users from running <strong>live streams</strong>`
      )
    }
  }
  ...
  private computeDefaultVideoPrivacy () {
    ...
    if (this.defaultPrivacy === VideoPrivacy.INTERNAL) {
      this.addExplanation(
        $localize`:bullet point of "BoomBoom will\:":Set the <strong>default video privacy</strong> to <strong>Internal</strong>`
      )
    } else if (this.defaultPrivacy === VideoPrivacy.PUBLIC) {
      this.addExplanation(
        $localize`:bullet point of "BoomBoom will\:":Set the <strong>default video privacy</strong> to <strong>Public</strong>`
      )
    }
  }
  ...
  private computeDefaultVideoCommentPolicy () {
    ...
    this.addExplanation($localize`:bullet point of "BoomBoom will\:":<strong>Require approval</strong> by default of new video comment`)
  }
  ...
  private computeP2P () {
    ...
    if (this.p2p === 'enabled') {
      this.addExplanation(
        $localize`:bullet point of "BoomBoom will\:":<strong>Enable P2P streaming</strong> by default for anonymous and new users`
      )
    } else {
      this.addExplanation(
        $localize`:bullet point of "BoomBoom will\:":<strong>Disable P2P streaming</strong> by default for anonymous and new users`
      )
    }
  }
  ...
  private computeFederation () {
    ...
    if (this.federation === 'enabled') {
      this.addExplanation(
        // eslint-disable-next-line max-len
        $localize`:bullet point of "BoomBoom will\:":<strong>Allow</strong> external platforms/users to <strong>subscribe</strong> to your content`
      )
    } else {
      this.addExplanation(
        // eslint-disable-next-line max-len
        $localize`:bullet point of "BoomBoom will\:":<strong>Prevent</strong> external platforms/users to <strong>subscribe to your content</strong>`
      )
    }
  }
  ...
  private computeKeepOriginalVideo () {
    ...
    if (this.keepOriginalVideo === 'enabled') {
      this.addExplanation($localize`:bullet point of "BoomBoom will\:":<strong>Save a copy</strong> of the uploaded video file`)
    }
  }
  ...
  private computeReplaceVideoFile () {
    ...
    if (this.allowReplaceFile === 'enabled') {
      this.addExplanation(
        // eslint-disable-next-line max-len
        $localize`:bullet point of "BoomBoom will\:":<strong>Allow</strong> your users <strong>to replace a video</strong> that has already been published`
      )
    }
  }
  ...
  private computeGlobalSearch () {
    ...
    if (this.globalSearch === 'enabled') {
      this.addExplanation(
        // eslint-disable-next-line max-len
        $localize`:bullet point of "BoomBoom will\:":Set <a href="https://sepiasearch.org" target="_blank">SepiaSearch</a> as <strong>default search engine</strong>`
      )
    }
  }
  ...
  private computeTranscription () {
    ...
    if (this.transcription === 'enabled') {
      this.addExplanation(
        // eslint-disable-next-line max-len
        $localize`:bullet point of "BoomBoom will\:":<strong>Enable automatic transcription</strong> of videos to create subtitles and improve accessibility`
      )
    }
  }
  ...
  private computeAuth () {
    ...
    const configStr =
      // eslint-disable-next-line max-len
      $localize`:bullet point of "BoomBoom will\:": The plugin <strong>must be configured</strong> after the pre-configuration wizard confirmation.`

    if (this.authType === 'ldap') {
      this.addExplanation(
        $localize`:bullet point of "BoomBoom will\:":Install the <strong>LDAP</strong> authentication plugin.` + configStr
      )

      this.plugins.push('boomboom-plugin-auth-ldap')
    } else if (this.authType === 'saml') {
      this.addExplanation(
        $localize`:bullet point of "BoomBoom will\:":Install the <strong>SAML 2.0</strong> authentication plugin.` + configStr
      )

      this.plugins.push('boomboom-plugin-auth-saml2')
    } else if (this.authType === 'oidc') {
      this.addExplanation(
        $localize`:bullet point of "BoomBoom will\:":Install the <strong>OpenID Connect</strong> authentication plugin.` + configStr
      )

      this.plugins.push('boomboom-plugin-auth-openid-connect')
    }
  }
