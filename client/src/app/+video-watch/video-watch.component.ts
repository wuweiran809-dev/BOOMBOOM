...
import { getPlaylistRSSFeeds, getVideoRSSFeeds, timeToInt } from '@boomboom/boomboom-core-utils'
import {
  HTMLServerConfig,
  HttpStatusCode,
  LiveVideo,
...
const debugLogger = debug('boomboom:watch:VideoWatchComponent')
...
export class VideoWatchComponent implements OnInit, OnDestroy {
  ...
  private boomboomSocket = inject(BoomBoomSocket)
  ...
  // Mobile drama episode strip (built from recommendations)
  episodes: Video[] = []
  readonly freeEpisodeCount = 3
  ...
  private readonly onPlayerTimeUpdate = () => {
    const player = this.boomboomPlayer?.getPlayer()
    if (!player) return
  ...
  async ngOnInit () {
    ...
    const constructorOptions = await this.hooks.wrapFun(
      this.buildBoomBoomPlayerConstructorOptions.bind(this),
    ...
    this.boomboomPlayer = new BoomBoomPlayer(constructorOptions)
  }

  ngOnDestroy () {
    this.unbindPlayerEventListeners()

    if (this.boomboomPlayer) this.boomboomPlayer.destroy()
  ...
  onRecommendations (videos: Video[]) {
    ...
    // Build the mobile episode strip: current video first, then recommendations
    this.episodes = [ this.video, ...videos ].slice(0, 24)
  ...
  isCurrentEpisode (ep: Video) {
    return this.video && ep?.uuid === this.video.uuid
  }

  isLockedEpisode (index: number) {
    return index >= this.freeEpisodeCount
  }

  goToEpisode (ep: Video) {
    if (!ep || this.isCurrentEpisode(ep)) return
    this.router.navigate([ '/w', ep.shortUUID || ep.uuid ])
  }

  padEpisodeNumber (index: number) {
  ...
  handleTimestampClicked (timestamp: number) {
    if (!this.boomboomPlayer || this.video.isLive) return

    const player = this.boomboomPlayer.getPlayer()
    if (!player) return

    this.boomboomPlayer.setCurrentTime(timestamp)
  ...
  private loadRouteQuery () {
    this.queryParamsSub = this.route.queryParams.subscribe(queryParams => {
      ...
      if (this.boomboomPlayer?.getPlayer() && start) {
        this.boomboomPlayer.setCurrentTime(parseInt(start, 10))
      }
    })
  }
  ...
  private async handleRequestError (err: any) {
    const errorBody = err.body as BoomBoomProblemDocument
  ...
  private async loadPlayer (options: {
    ...
    const loadOptions = await this.hooks.wrapFun(
      this.buildBoomBoomPlayerLoadOptions.bind(this),
      params,
      'video-watch',
    ...
    this.zone.runOutsideAngular(async () => {
      await this.boomboomPlayer.load(loadOptions)
  ...
  private unbindPlayerEventListeners (player?: VideojsPlayer) {
    const playerInstance = player ?? this.boomboomPlayer?.getPlayer()
  ...
  private buildBoomBoomPlayerConstructorOptions (options: {
  ...
  private buildBoomBoomPlayerLoadOptions (options: {
    video: VideoDetails
    liveVideo: LiveVideo
    videoCaptions: VideoCaption[]
    videoChapters: VideoChapter[]
    ...
    } satisfies BoomBoomPlayerLoadOptions
  }
  ...
  private async subscribeToLiveEventsIfNeeded (oldVideo: VideoDetails, newVideo: VideoDetails) {
    ...
    if (oldVideo && oldVideo.id !== newVideo.id) {
      this.boomboomSocket.unsubscribeLiveVideos(oldVideo.id)
    }

    if (!newVideo.isLive) return

    await this.boomboomSocket.subscribeToLiveVideosSocket(newVideo.id)
  }

  private buildLiveEventsSubscription () {
    return this.boomboomSocket.getLiveVideosObservable()
  ...
  private updatePlayerOnNoLive ({ hasPlayed }: { hasPlayed: boolean }) {
    this.boomboomPlayer.unload()
    this.boomboomPlayer.disable()

    if (hasPlayed || !this.video.isNSFWBlurForUser(this.authUser || this.anonymousUser, this.serverConfig)) {
      this.boomboomPlayer.setPoster(this.video.thumbnails)
    }
  }
