...
import { HTMLServerConfig, VideoState } from '@boomboom/boomboom-models'
...
export class ThumbnailManagerComponent implements OnInit, ControlValueAccessor {
  ...
  selectFromVideo () {
    ...
    runInInjectionContext(this.environmentInjector, () => {
      afterNextRender(() => {
        try {
          this.player = new BoomBoomPlayer(this.embed().getIframe())
        } catch (err: unknown) {
          this.notifier.error('Error creating BoomBoom embed: ' + (err as Error).message)
