...
import { Notifier, BoomBoomRouterService, ServerService } from '@app/core'
...
import { secondsToTime } from '@boomboom/boomboom-core-utils'
import {
  HttpStatusCode,
  LiveVideoSession,
  VideoStatsOverall,
...
export class VideoStatsComponent implements OnInit {
  ...
  resetZoom () {
    this.boomboomRouter.silentNavigate([], {})
  ...
  onDateFilterChange () {
    ...
    if (idParts.length === 2) {
      return this.boomboomRouter.silentNavigate([], { startDate: idParts[0], endDate: idParts[1] })
    }
  }
        ...
        zoom: {
          zoom: {
            ...
            onZoomComplete: ({ chart }) => {
              ...
              this.boomboomRouter.silentNavigate([], { startDate, endDate })
              this.addAndSelectCustomDateFilter()
            }
          },
          limits: {
