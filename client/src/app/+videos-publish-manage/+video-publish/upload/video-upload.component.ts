...
import { AuthService, CanComponentDeactivate, HooksService, MetaService, Notifier, ScreenService, ServerService } from '@app/core'
...
export class VideoUploadComponent implements OnInit, OnDestroy, AfterViewInit, CanComponentDeactivate {
  ...
  // Mobile drama-compose state
  mobileTitle = ''
  mobileDescription = ''
  selectedGenre = '都市'
  paidUnlock = false
  readonly genres = [ '都市', '甜宠', '逆袭', '古装', '悬疑' ]
  ...
  // Opens the native file picker, reusing the real upload input
  triggerFilePick () {
  ...
  private buildVideoFilename (filename: string) {
    // On mobile compose, prefer the title the user typed on the drama-publish screen
    if (this.mobileTitle?.trim()) {
      return truncate(this.mobileTitle.trim(), { length: 110, omission: '' })
    }
