...
import { AVAILABLE_LOCALES } from '@boomboom/boomboom-core-utils'
import { MetaGuard } from './core'
import { mobileRedirectGuard } from './core/routing/mobile-redirect.guard'
...
const routes: Routes = [
  ...
  },
  {
    ...
    canActivate: [ mobileRedirectGuard ],
    canActivateChild: [ MetaGuard ],
    data: { mobileTarget: '/m/discover' }
  },

  {
    path: 'feed',
    loadChildren: () => import('./+video-feed/routes'),
    canActivate: [ mobileRedirectGuard ],
    canActivateChild: [ MetaGuard ],
    data: { mobileTarget: '/m/feed' }
  },

  // BoomBoom 移动端「动感海报风」独立模块
  {
    path: 'm',
    loadChildren: () => import('./+m/m.routes'),
    canActivateChild: [ MetaGuard ]
  },
