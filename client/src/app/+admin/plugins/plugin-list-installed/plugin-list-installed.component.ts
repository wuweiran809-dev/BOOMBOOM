...
import { BoomBoomPlugin, PluginType, PluginType_Type } from '@boomboom/boomboom-models'
...
export class PluginListInstalledComponent implements OnInit {
  ...
  async update (plugin: BoomBoomPlugin) {
    const pluginKey = this.getPluginKey(plugin)
  ...
  getShowRouterLink (plugin: BoomBoomPlugin) {
  ...
  private getPluginKey (plugin: BoomBoomPlugin) {
    return plugin.name + plugin.type
  }

  private isMajorUpgrade (plugin: BoomBoomPlugin) {
