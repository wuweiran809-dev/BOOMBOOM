...
import { VideoRedundanciesTarget, VideoRedundancy, VideosRedundancyStats } from '@boomboom/boomboom-models'
...
export class VideoRedundanciesListComponent implements OnInit {
  ...
  private loadSelectLocalStorage () {
    const displayType = boomboomLocalStorage.getItem(VideoRedundanciesListComponent.LS_DISPLAY_TYPE)
    if (displayType) this.displayType = displayType as VideoRedundanciesTarget
  }

  private saveSelectLocalStorage () {
    boomboomLocalStorage.setItem(VideoRedundanciesListComponent.LS_DISPLAY_TYPE, this.displayType)
  }
