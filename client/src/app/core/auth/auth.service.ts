...
import { HttpStatusCode, OAuthClientLocal, User, UserLogin, UserRefreshToken, MyUser as UserServerModel } from '@boomboom/boomboom-models'
import { logger, OAuthUserTokens, objectToUrlEncoded, boomboomLocalStorage, BoomBoomReconnectError } from '@root-helpers/index'
...
export class AuthService {
  ...
  private clientId: string = boomboomLocalStorage.getItem(AuthService.LS_OAUTH_CLIENT_KEYS.CLIENT_ID)
  private clientSecret: string = boomboomLocalStorage.getItem(AuthService.LS_OAUTH_CLIENT_KEYS.CLIENT_SECRET)
    ...
    this.http.get<OAuthClientLocal>(AuthService.BASE_CLIENT_URL)
      ...
      .subscribe({
        next: res => {
          ...
          boomboomLocalStorage.setItem(AuthService.LS_OAUTH_CLIENT_KEYS.CLIENT_ID, this.clientId)
          boomboomLocalStorage.setItem(AuthService.LS_OAUTH_CLIENT_KEYS.CLIENT_SECRET, this.clientSecret)
...
Ensure you have correctly configured BoomBoom (config/ directory), in particular the "webserver" section.`
          }
  ...
  login (options: {
    ...
    if (otpToken) headers = headers.set('x-boomboom-otp', otpToken)
  ...
  logout () {
    ...
    obs.subscribe({
      ...
      error: err => {
        if (!(err instanceof BoomBoomReconnectError)) {
          logger.error(err)
        }
      }
    })
  ...
  refreshAccessToken () {
    ...
    if (!this.getAccessToken()) return throwError(() => new BoomBoomReconnectError($localize`You need to reconnect`, true))
    ...
    this.refreshingTokenObservable = this.http.post<UserRefreshToken>(AuthService.BASE_TOKEN_URL, body, { headers })
      .pipe(
        ...
        catchError(err => {
          ...
          throw new BoomBoomReconnectError($localize`You need to reconnect`, true)
        }),
        share()
      )
  ...
  isOTPMissingError (err: HttpErrorResponse) {
    ...
    if (err.headers.get('x-boomboom-otp') !== 'required; app') return false
