...
import { HTMLServerConfig, ServerConfig, ServerConfigTheme } from '@boomboom/boomboom-models'
...
export class ThemeService {
  ...
  getDefaultThemeItem () {
    return {
      ...
      id: 'default',
      description: $localize`BoomBoom selects the appropriate theme depending on web browser preferences`
    }
  }
  ...
  buildAvailableThemes () {
    return [
      ...this.serverConfig.theme.builtIn.map(t => {
        if (t.name === 'boomboom-core-dark-brown') {
          return { id: t.name, label: $localize`Dark (Brown)` }
        }

        if (t.name === 'boomboom-core-light-beige') {
  ...
  getCurrentThemeName () {
    ...
    if (window.matchMedia?.('(prefers-color-scheme: dark)').matches) {
      return 'boomboom-core-dark-brown' satisfies ServerConfig['theme']['builtIn'][0]['name']
    }
