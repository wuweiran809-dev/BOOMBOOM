...
import { BoomBoomRouterService } from './boomboom-router.service'

const debugLogger = debug('boomboom:main:ScrollService')
...
export class ScrollService {
  ...
  private produceScroll () {
    ...
    this.boomboomRouter.getNavigationEndEvents().pipe(pairwise())
      .subscribe(([ e1, e2 ]) => {
        try {
          ...
          if (this.boomboomRouter.hasRouteSetting(RouterSetting.DISABLE_SCROLL_RESTORE)) {
  ...
  private consumeScroll () {
    ...
    this.boomboomRouter.getScrollEvents().subscribe(e => {
