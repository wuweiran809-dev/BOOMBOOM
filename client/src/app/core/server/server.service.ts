...
import { getCompleteLocale, isDefaultLocale, boomboomTranslate } from '@boomboom/boomboom-core-utils'
import {
  HTMLServerConfig,
  ServerConfig,
  ServerStats,
...
export class ServerService {
        ...
        map(({ data, translations }) => {
          const hashToPopulate: ConstantLabel<T>[] = Object.keys(data)
            .map(dataKey => {
              ...
              return {
                id,
                label: boomboomTranslate(label, translations)
              }
            })
  ...
  private loadHTMLConfigLocally () {
    const configString = (window as any)['BoomBoomServerConfig']
    if (!configString) {
      throw new Error('Could not find BoomBoomServerConfig in HTML')
    }
