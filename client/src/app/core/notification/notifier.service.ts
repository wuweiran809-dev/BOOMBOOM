...
import { BoomBoomHTTPError, BoomBoomReconnectError } from '@root-helpers/errors'
...
export class Notifier {
  ...
  handleError (err: Error) {
    if (err instanceof BoomBoomReconnectError && err.alreadyNotified) return

    const text = this.buildErrorText(err)

    if (err instanceof BoomBoomHTTPError) {
  ...
  private buildErrorText (err: Error) {
    if (
      (err instanceof BoomBoomHTTPError && err.status === HttpStatusCode.BAD_GATEWAY_502) ||
