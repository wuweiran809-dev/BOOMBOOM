...
import { EventHandler, BoomBoomResolution, BoomBoomTextTrack, PlayerEventType } from './definitions'
...
/**
 * Allows for programmatic control of a BoomBoom embed running in an <iframe>
 * within a web page.
 */
export class BoomBoomPlayer {
  ...
  /**
   * Construct a new BoomBoomPlayer for the given BoomBoom embed iframe.
   * Optionally provide a `scope` to ensure that messages are not crossed
   * between multiple BoomBoom embeds. The string passed here must match the
   ...
   * Get video captions
   */
  async getCaptions (): Promise<BoomBoomTextTrack[]> {
    return this.sendMessage<undefined, BoomBoomTextTrack[]>('getCaptions')
  }
   ...
   * resolutions change.
   */
  async getResolutions (): Promise<BoomBoomResolution[]> {
    return this.sendMessage<undefined, BoomBoomResolution[]>('getResolutions')
  }
...
(window as any)['BoomBoomPlayer'] = BoomBoomPlayer
