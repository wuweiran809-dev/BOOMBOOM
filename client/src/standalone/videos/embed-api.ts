...
import { BoomBoomResolution, BoomBoomTextTrack } from '../embed-player-api/definitions'
import { BoomBoomEmbed } from './embed'
import './embed.scss'
import { BoomBoomPlayer } from '@boomboom/player'

/**
 * Embed API exposes control of the embed player to the outside world via
 * JSChannels and window.postMessage
 */
export class BoomBoomEmbedApi {
  private channel: Channel.MessagingChannel
  private isReady = false
  private resolutions: BoomBoomResolution[] = []
  ...
  constructor (private readonly embed: BoomBoomEmbed) {
  }
  ...
  private get player (): ReturnType<BoomBoomPlayer['getPlayer']> {
  ...
  private setResolution (resolutionId: number) {
    ...
    this.player.boomboomResolutions().select({ id: resolutionId, fireCallback: true })
  }

  private getCaptions (): BoomBoomTextTrack[] {
  ...
  private setupStateTracking () {
    ...
    // BoomBoom specific capabilities
    this.player.boomboomResolutions().on('resolutions-added', () => this.loadResolutions())
    this.player.boomboomResolutions().on('resolutions-changed', () => this.loadResolutions())
  ...
  private loadResolutions () {
    this.resolutions = this.player.boomboomResolutions().getResolutions()
      .map(r => ({
        id: r.id,
        label: r.label,
        active: r.selected,
