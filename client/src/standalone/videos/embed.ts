...
import type { BoomBoomPlayer, VideojsPlayer } from '@boomboom/player'
import { BoomBoomServerError } from '@pt-types'
...
import { BoomBoomEmbedApi } from './embed-api'
import './embed.scss'
import {
  AuthHTTP,
  LiveManager,
...
export class BoomBoomEmbed {
  player: VideojsPlayer
  api: BoomBoomEmbedApi = null

  config: HTMLServerConfig

  private translationsPromise: Promise<{ [id: string]: string }>
  private BoomBoomPlayerManagerModulePromise: Promise<any>
  ...
  private readonly boomboomPlugin: BoomBoomPlugin
  private readonly boomboomTheme: BoomBoomTheme
  private readonly playerHTML: PlayerHTML
  private readonly playerOptionsBuilder: PlayerOptionsBuilder
  private readonly liveManager: LiveManager

  private boomboomPlayer: BoomBoomPlayer
  ...
  constructor (videoWrapperId: string) {
    ...
    this.boomboomPlugin = new BoomBoomPlugin(this.http)
    this.boomboomTheme = new BoomBoomTheme(this.boomboomPlugin)
    this.playerHTML = new PlayerHTML(videoWrapperId)
    this.playerOptionsBuilder = new PlayerOptionsBuilder(this.playerHTML, this.videoFetcher, this.boomboomPlugin)
    this.liveManager = new LiveManager(this.playerHTML)
    this.requiresPassword = false

    try {
      this.config = JSON.parse((window as any)['BoomBoomServerConfig'])
    } catch (err) {
  ...
  static async main () {
    ...
    const embed = new BoomBoomEmbed(videoContainerId)
    await embed.init()
  }
  ...
  async init () {
    ...
    this.BoomBoomPlayerManagerModulePromise = import('@boomboom/player')
    ...
    this.boomboomTheme.loadThemeStyle(this.config)
  ...
  private initializeApi () {
    ...
    this.api = new BoomBoomEmbedApi(this)
    this.api.initialize()
  }
  ...
  private async loadVideoAndBuildPlayer (options: {
    ...
    try {
      ...
      if (this.player?.usingPlugin('boomboom')) {
        this.player.boomboom().displayFatalError({ error: err, log: false, isTechnicalError: false })
        return
      }
  ...
  private async buildVideoPlayer (options: {
    ...
    if (!this.alreadyInitialized) {
      this.player = this.boomboomPlayer.getPlayer()
    ...
    this.boomboomPlugin.getPluginsManager().runHook(
  ...
  private endLive (video: VideoDetails, translations: Translations) {
    ...
    this.boomboomPlayer.setPoster(video.thumbnails)
  }

  private async handlePasswordError (err: BoomBoomServerError) {
  ...
  private async buildPlayerIfNeeded () {
    ...
    playerElement.className = 'video-js vjs-boomboom-skin'
    ...
    const [ { BoomBoomPlayer, videojs } ] = await Promise.all([
      this.BoomBoomPlayerManagerModulePromise,

      this.translationsPromise.then(translations => {
        this.boomboomPlugin.init(translations)
        this.boomboomPlugin.loadPlugins(this.config)
        this.boomboomTheme.loadThemePlugins(this.config)

        return this.boomboomPlugin.ensurePluginsAreLoaded()
      })
    ])
