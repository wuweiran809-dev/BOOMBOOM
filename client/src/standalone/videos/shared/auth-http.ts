import { HttpStatusCode, OAuth2ErrorCode, OAuth2ErrorCodeType, UserRefreshToken } from '@boomboom/boomboom-models'
...
export class AuthHTTP {
  ...
  constructor (private readonly serverUrl: string, private readonly language: string) {
    this.userOAuthTokens = OAuthUserTokens.getUserTokens(boomboomLocalStorage)
  ...
  fetch (url: string, { optionalAuth, method }: { optionalAuth: boolean, method?: string }, videoPassword?: string) {
    ...
    if (isSameOrigin(this.serverUrl, url)) {
      if (this.language) this.headers.set('x-boomboom-language', this.language)

      if (videoPassword) this.headers.set('x-boomboom-video-password', videoPassword)
    ...
    return fetch(url, options)
      .then((res: Response) => {
        ...
        const refreshingTokenPromise = new Promise<void>((resolve, reject) => {
          const clientId: string = boomboomLocalStorage.getItem(this.LS_OAUTH_CLIENT_KEYS.CLIENT_ID)
          const clientSecret: string = boomboomLocalStorage.getItem(this.LS_OAUTH_CLIENT_KEYS.CLIENT_SECRET)
          ...
          fetch('/api/v1/users/token', {
            ...
            if (!obj || obj.code === OAuth2ErrorCode.INVALID_GRANT) {
              OAuthUserTokens.flushLocalStorage(boomboomLocalStorage)
            ...
            OAuthUserTokens.saveToLocalStorage(boomboomLocalStorage, this.userOAuthTokens)
