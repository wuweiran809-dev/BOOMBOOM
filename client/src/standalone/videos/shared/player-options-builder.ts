...
import {
  ...
  logger,
  boomboomLocalStorage,
  UserLocalStorageKeys,
  videoRequiresUserAuth
} from '../../../root-helpers'
import { HLSOptions, BoomBoomPlayerConstructorOptions, BoomBoomPlayerLoadOptions, VideoJSCaption } from '../../player'
import { BoomBoomPlugin } from './boomboom-plugin'
...
export class PlayerOptionsBuilder {
  ...
  loadCommonParams () {
    try {
      ...
      this.boomboomLink = getParamToggle(params, 'boomboomLink', true)
  ...
  getPlayerConstructorOptions (options: {
    ...
    return {
      ...
      boomboomLink: () => this.boomboomLink,
      ...
      pluginsManager: this.boomboomPlugin.getPluginsManager(),
  ...
  }): Promise<BoomBoomPlayerLoadOptions> {
    const {
      video,
      captionsResponse,
      videoFileToken,
  ...
  private async buildCaptions (captionsResponse: Response, translations: Translations): Promise<VideoJSCaption[]> {
    if (captionsResponse.ok) {
      ...
      return data.map((c: VideoCaption) => ({
        label: boomboomTranslate(c.language.label, translations),
  ...
  private buildDockOptions (videoInfo: VideoDetails) {
    ...
    const description = this.hasWarningTitle() && this.hasP2PEnabled()
      ? boomboomTranslate('Watching this video may reveal your IP address to others.')
      : undefined
  ...
  private isP2PEnabled (config: HTMLServerConfig, video: Video) {
    const userP2PEnabled = getBoolOrDefault(
      boomboomLocalStorage.getItem(UserLocalStorageKeys.P2P_ENABLED),
