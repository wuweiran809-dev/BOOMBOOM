...
import { BoomBoomMobileButtons } from './boomboom-mobile-buttons'

const debugLogger = debug('boomboom:player:mobile')
...
class BoomBoomMobilePlugin extends Plugin {
  private static readonly DOUBLE_TAP_DELAY_MS = 250
  private static readonly SET_CURRENT_TIME_DELAY = 750

  declare private boomBoomMobileButtons: BoomBoomMobileButtons
  ...
  constructor (player: VideojsPlayer) {
    ...
    this.boomBoomMobileButtons = player.addChild('BoomBoomMobileButtons', { reportTouchActivity: false }) as BoomBoomMobileButtons
  ...
  dispose () {
    ...
    if (this.onMobileButtonTouchStartHandler) {
      const el = this.boomBoomMobileButtons?.el() as HTMLElement
  ...
  private initTouchStartEvents () {
    const handleTouchStart = (event: TouchEvent, onlyDetectDoubleTap = false) => {
      ...
      if (
        this.doubleTapping ||
        (this.lastTapEvent && event.timeStamp - this.lastTapEvent.timeStamp < BoomBoomMobilePlugin.DOUBLE_TAP_DELAY_MS)
      ) {
      ...
      }, BoomBoomMobilePlugin.DOUBLE_TAP_DELAY_MS)
  ...
  private onDoubleTap (event: TouchEvent) {
    ...
    this.boomBoomMobileButtons.displayFastSeek(this.seekAmount)
  ...
  private scheduleSetCurrentTime () {
    ...
    this.setCurrentTimeTimeout = setTimeout(() => {
      ...
      this.seekAmount = 0
      this.boomBoomMobileButtons.displayFastSeek(0)
    ...
    }, BoomBoomMobilePlugin.SET_CURRENT_TIME_DELAY)
  }
}

videojs.registerPlugin('boomboomMobile', BoomBoomMobilePlugin)
export { BoomBoomMobilePlugin }
