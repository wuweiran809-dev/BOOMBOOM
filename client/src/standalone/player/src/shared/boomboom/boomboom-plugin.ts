import { timeToInt } from '@boomboom/boomboom-core-utils'
import { VideoStatsUserAgentDevice, VideoView, VideoViewEvent } from '@boomboom/boomboom-models'
...
import {
  ...
  saveVolumeInStore
} from '../../boomboom-player-local-storage'
import type {
  BoomBoomPluginOptions,
  VideojsAutoplay,
...
const debugLogger = debug('boomboom:player:boomboom')

const Plugin = videojs.getPlugin('plugin') as typeof VideojsPlugin

class BoomBoomPlugin extends Plugin {
  ...
  constructor (player: VideojsPlayer, private readonly options: BoomBoomPluginOptions) {
    super(player)
  ...
  private updateControlBar () {
    ...
    if (this.options.isLive()) {
      ...
      this.player.controlBar.getChild('boomBoomLiveDisplay').show()
      ...
      this.player.controlBar.getChild('boomBoomLiveDisplay').hide()
    }
...
videojs.registerPlugin('boomboom', BoomBoomPlugin)
export { BoomBoomPlugin }
