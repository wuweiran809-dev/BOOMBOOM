...
class P2PInfoButton extends Button {
  createEl () {
    const div = videojs.dom.createEl('div', { className: 'vjs-boomboom' })
    const subDivP2P = videojs.dom.createEl('div', {
      className: 'vjs-boomboom-hidden' // Hide the stats before we get the info
    }) as HTMLDivElement
    ...
    const subDivHttp = videojs.dom.createEl('div', { className: 'vjs-boomboom-hidden' }) as HTMLElement
    ...
    this.player_.on('network-info', (_event: any, data: PlayerNetworkInfo) => {
      ...
      subDivP2P.className = 'vjs-boomboom-displayed'
      subDivHttp.className = 'vjs-boomboom-hidden'
    ...
    this.player_.on('network-info', (_event: any, data: PlayerNetworkInfo) => {
      ...
      // We are in HTTP mode
      subDivHttp.className = 'vjs-boomboom-displayed'
      subDivP2P.className = 'vjs-boomboom-hidden'
