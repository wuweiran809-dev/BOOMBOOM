import { exists, getResolutionAndFPSLabel, getResolutionLabel, timeToInt } from '@boomboom/boomboom-core-utils'
import { LiveVideoLatencyMode } from '@boomboom/boomboom-models'
...
import { getAverageBandwidthInStore } from '../../boomboom-player-local-storage'
import {
  HLSPluginOptions,
...
const debugLogger = debug('boomboom:player:hls')

type ConstructorOptions =
  & Pick<BoomBoomPlayerConstructorOptions, 'pluginsManager' | 'serverUrl' | 'authorizationHeader' | 'stunServers'>
  & Pick<
    BoomBoomPlayerLoadOptions,
    | 'videoPassword'
    | 'requiresUserAuth'
...
export class HLSOptionsBuilder {
  ...
  private getP2PMediaLoaderOptions (options: {
    ...
    if (
      ...
      boomboomLocalStorage.getItem('boomboom-videojs-p2p-consume-only') === 'true' // Use for E2E testing
    ) {
    ...
    const loaderOptions: Partial<StreamConfig> = {
      ...
      httpRequestSetup: (segmentUrlArg, segmentByteRange, requestAbortSignal, requestByteRange) => {
        ...
        if (isSameOrigin(this.options.serverUrl, segmentUrl)) {
          if (requiresPassword) {
            headers.set('x-boomboom-video-password', this.options.videoPassword())
    ...
    const base: HLSPluginOptions = {
      ...
      xhrSetup: (xhr, url) => {
        ...
        if (isSameOrigin(this.options.serverUrl, url)) {
          if (requiresPassword) {
            xhr.setRequestHeader('x-boomboom-video-password', this.options.videoPassword())
          } else if (requiresUserAuth) {
