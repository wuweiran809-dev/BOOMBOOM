...
import { HLSPluginOptions, HlsjsConfigHandlerOptions, BoomBoomResolution, VideoJSTechHLS, VideojsPlayer, VideojsPlugin } from '../../types'
...
export class Html5Hlsjs {
  ...
  private _removeQuality (index: number) {
    ...
    this.player.boomboomResolutions().remove(index)
  ...
  private _notifyVideoQualities () {
    ...
    const resolutions: BoomBoomResolution[] = []
    ...
    if (preferredResolution) {
      this.player.boomboomResolutions().select({ id: preferredResolution.id, fireCallback: true })
    }
  }

  private pickPreferredResolution (resolutions: BoomBoomResolution[]) {
  ...
  private registerLevelEventSwitch () {
    this.hls.on(Hlsjs.Events.LEVEL_SWITCHING, (_e, data: LevelSwitchingData) => {
      ...
      this.player.boomboomResolutions().select({ id: resolutionId, autoResolutionChosenId, fireCallback: false })
    })
  }
