import { removeQueryParams } from '@boomboom/boomboom-core-utils'
...
const debugLogger = debug('boomboom:player:segment-validator')
...
export class SegmentValidator {
  ...
  private fetchSha256Segments (): Promise<SegmentsJSON> {
    ...
    if (isSameOrigin(this.options.serverUrl, this.options.segmentsSha256Url)) {
      if (this.options.requiresPassword) headers = { 'x-boomboom-video-password': this.options.videoPassword() }
