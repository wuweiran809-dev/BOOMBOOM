...
class BoomBoomHotkeysPlugin extends Plugin {
  ...
  private buildHandlers () {
    const handlers: KeyHandler[] = [
      ...
      // Increase volume
      {
        ...
        cb: e => {
          e.preventDefault()
          this.player.volume(this.player.volume() + BoomBoomHotkeysPlugin.VOLUME_STEP)
        }
      },
    ...
    const handlers: KeyHandler[] = [
      // Rewind
      {
        ...
        cb: e => {
          ...
          const target = Math.max(0, this.player.currentTime() - BoomBoomHotkeysPlugin.SEEK_STEP)
      ...
      // Forward
      {
        ...
        cb: e => {
          ...
          const target = Math.min(this.player.duration(), this.player.currentTime() + BoomBoomHotkeysPlugin.SEEK_STEP)
          this.player.currentTime(target)
        }
      }
    ]
...
videojs.registerPlugin('boomBoomHotkeysPlugin', BoomBoomHotkeysPlugin)
export { BoomBoomHotkeysPlugin }
