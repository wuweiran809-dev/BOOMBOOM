...
let boomboomLocalStorage: Storage
let boomboomSessionStorage: Storage

function reinitStorage () {
  const instanceLocalStorage = new MemoryStorage()
  const instanceSessionStorage = new MemoryStorage()

  boomboomLocalStorage = proxify(instanceLocalStorage)
  boomboomSessionStorage = proxify(instanceSessionStorage)
}

try {
  boomboomLocalStorage = localStorage
  boomboomSessionStorage = sessionStorage
} catch (err) {
...
if (!boomboomLocalStorage || !boomboomSessionStorage) {
  reinitStorage()
}

export {
  boomboomLocalStorage,
  boomboomSessionStorage
}
