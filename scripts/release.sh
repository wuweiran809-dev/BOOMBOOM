...
boomboom_directory=$(basename $(pwd))
...
directory_name="boomboom-$version"
zip_name="boomboom-$version.zip"
tar_name="boomboom-$version.tar.xz"
...
(
  ...
  # temporary setup
  cd ..
  ln -s "$boomboom_directory" "$directory_name"

  # archive creation + signing
  zip -9 -r "$boomboom_directory/$zip_name" "${directories_to_archive[@]}"
  gpg --armor --detach-sign -u "$maintainer_public_key" "$boomboom_directory/$zip_name"
  XZ_OPT="-e9 -T0" tar cfJ "$boomboom_directory/$tar_name" "${directories_to_archive[@]}"
  gpg --armor --detach-sign -u "$maintainer_public_key" "$boomboom_directory/$tar_name"
...
(
  ...
  if [ -z "$github_prerelease_option" ]; then
    github-release release --user chocobozzz --repo boomboom --tag "$version" --name "$version" --description "$changelog"
  else
    github-release release --user chocobozzz --repo boomboom --tag "$version" --name "$version" --description "$changelog" "$github_prerelease_option"
  fi
  ...
  github-release upload --user chocobozzz --repo boomboom --tag "$version" --name "$zip_name" --file "$zip_name"
  github-release upload --user chocobozzz --repo boomboom --tag "$version" --name "$zip_name.asc" --file "$zip_name.asc"
  github-release upload --user chocobozzz --repo boomboom --tag "$version" --name "$tar_name" --file "$tar_name"
  github-release upload --user chocobozzz --repo boomboom --tag "$version" --name "$tar_name.asc" --file "$tar_name.asc"
  ...
  if [ -z "$github_prerelease_option" ]; then
      ...
      scp "$tar_name" "$tar_name.asc" framasoft_builds_joinboomboom@builds.joinboomboom.org:/var/www/clients/client1/web30/web/release
  fi
)
