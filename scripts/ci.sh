...
if [ "$1" = "types-package" ]; then
    ...
    pnpm run --filter=@boomboom/tests install-dependencies:transcription
    ...
    boomboomRunnerFiles=$(findTestFiles ./packages/tests/dist/boomboom-runner)

    runJSTest "$1" 1 $externalPluginsFiles
    MOCHA_PARALLEL=true runJSTest "$1" $((2*$speedFactor)) $boomboomRunnerFiles
elif [ "$1" = "lint" ]; then
