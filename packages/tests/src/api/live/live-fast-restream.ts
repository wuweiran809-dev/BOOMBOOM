...
import { LiveVideoCreate, VideoPrivacy } from '@boomboom/boomboom-models'
import {
  cleanupTests,
  createSingleServer,
  BoomBoomServer,
...
describe('Fast restream in live', function () {
  ...
  async function ensureLastLiveWorks (liveId: string) {
    // Equivalent to BOOMBOOM_TEST_CONSTANTS_VIDEO_LIVE_CLEANUP_DELAY
    for (let i = 0; i < 100; i++) {
  ...
  before(async function () {
    ...
    const env = { BOOMBOOM_TEST_CONSTANTS_VIDEO_LIVE_CLEANUP_DELAY: '10000' }
