...
import { BoomBoomServer, cleanupTests, waitJobs } from '@boomboom/boomboom-server-commands'
...
describe('Test views user agent stats', function () {
  ...
  it('Should use a null value if device is not known by BoomBoom', async function () {
    await server.views.simulateView({
      id: videoUUID,
      sessionId: buildUUID(),
      client: 'Chrome',
