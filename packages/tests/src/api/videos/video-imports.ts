...
import { CustomConfig, HttpStatusCode, Video, VideoImportState, VideoPrivacy, VideoResolution, VideoState } from '@boomboom/boomboom-models'
import { areHttpImportTestsDisabled, areYoutubeImportTestsDisabled } from '@boomboom/boomboom-node-utils'
import {
  BoomBoomServer,
  cleanupTests,
...
import { DeepPartial } from '@boomboom/boomboom-typescript-utils'
...
async function checkVideosServer1 (server: BoomBoomServer, idHttp: string | undefined, idMagnet: string, idTorrent: string) {
  if (idHttp) {
  ...
  expect(videoMagnet.name).to.contain('super boomboom2 video')
}

async function checkVideoServer2 (server: BoomBoomServer, id: number | string) {
...
describe('Test video imports', function () {
  ...
  function runSuite (mode: 'youtube-dl' | 'yt-dlp') {
    describe('Import ' + mode, function () {
      ...
      it('Should import a magnet video on server 1', async function () {
        ...
        expect(video.name).to.equal('super boomboom2 video')
      })
      ...
      it('Should list the videos to import in my videos on server 1', async function () {
        ...
        const expected = [ 'super boomboom2 video', '你好 世界 720p.mp4' ]
      ...
      it('Should list the videos to import in my imports on server 1', async function () {
...

        {
          ...
          expect(videoImport.torrentName).to.be.null
          expect(videoImport.video.name).to.equal('super boomboom2 video')
        }
      ...
      it('Should search in my imports', async function () {
        {
          const { total, data } = await servers[0].videoImports.listMyVideoImports({ search: 'boomboom2' })
          expect(total).to.equal(1)
          expect(data).to.have.lengthOf(1)

          expect(data[0].magnetUri).to.equal(FIXTURE_URLS.magnet)
          expect(data[0].video.name).to.equal('super boomboom2 video')
        }
      ...
      it('Should import a boomboom video', async function () {
        this.timeout(120_000)

        const toTest = [ FIXTURE_URLS.boomboomLong ]

        // TODO: include boomboom_short when https://github.com/ytdl-org/youtube-dl/pull/29475 is merged
        if (mode === 'yt-dlp') {
          toTest.push(FIXTURE_URLS.boomboomShort)
        }
  ...
  describe('Auto update', function () {
    ...
    function quickBoomBoomImport () {
      const attributes = {
        targetUrl: FIXTURE_URLS.boomboomLong,
