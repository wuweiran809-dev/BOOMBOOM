...
import { HttpStatusCode, HttpStatusCodeType, LiveVideo, VideoDetails, VideoPrivacy, VideoResolution } from '@boomboom/boomboom-models'
import {
  cleanupTests,
  createSingleServer,
    ...
    async function checkVideoFiles (options: {
      ...
      for (const file of getAllFiles(video)) {
        for (const url of [ file.fileUrl, file.fileDownloadUrl ]) {
          ...
          if (videoPassword) {
            const headers = { 'x-boomboom-video-password': videoPassword }
  ...
  describe('Live static file path and check', function () {
    ...
    async function checkLiveFiles (options: { live: LiveVideo, liveId: string, videoPassword?: string }) {
      ...
      for (const url of [ hls.playlistUrl, hls.segmentsSha256Url ]) {
        ...
        if (videoPassword) {
          await makeRawRequest({ url, headers: { 'x-boomboom-video-password': videoPassword }, expectedStatus: HttpStatusCode.OK_200 })
