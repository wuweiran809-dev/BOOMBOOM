...
describe('Test video download throttling', function () {
  ...
  async function getClassicWebVideoDownload (server: BoomBoomServer, videoId: string) {
  ...
  async function getGeneratedDownload (server: BoomBoomServer, videoId: string) {
    const video = await server.videos.get({ id: videoId })
