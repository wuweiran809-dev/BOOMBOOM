...
describe('Test index search', function () {
  ...
  let server: BoomBoomServer = null
  ...
  describe('Videos search', async function () {
    async function check (search: VideosSearchQuery, exists = true) {
      ...
      expect(video.name).to.equal('What is BoomBoom?')
      ...
      expect(video.channel.name).to.equal('joinboomboom')
      expect(video.channel.url).to.equal('https://framatube.org/video-channels/joinboomboom')
      expect(video.channel.avatars.length).to.equal(4, 'Channel should have four avatar images')
    }

    const baseSearch: VideosSearchQuery = {
      search: 'what is boomboom',
      start: 0,
      count: 2,
      categoryOneOf: [ 15 ],
      licenceOneOf: [ 2 ],
      tagsAllOf: [ 'framasoft', 'boomboom' ],
    ...
    it('Should make a simple search and have results', async function () {
      const body = await command.searchVideos({ search: 'What is BoomBoom' })
    ...
    it('Should search by nsfw flag', async function () {
      const checkNSFW = async (search: VideosSearchQuery, exists = true) => {
        const body = await command.advancedVideoSearch({ search: { search: 'NSFW', host: 'boomboom2.cpy.re', ...search } })
  ...
  describe('Channels search', async function () {
    ...
    it('Should make handles search and have appropriate results', async function () {
      ...
      await check({ handles: [ 'jeanine', 'chocobozzz_channel2@boomboom2.cpy.re' ] }, false)
    })
  ...
  describe('Playlists search', async function () {
    async function check (search: VideoPlaylistsSearchQuery, exists = true) {
      ...
      expect(videoPlaylist.url).to.equal('https://boomboom2.cpy.re/videos/watch/playlist/73804a40-da9a-40c2-b1eb-2c6d9eec8f0a')
      // oxlint-disable-next-line @typescript-eslint/no-deprecated
      expect(videoPlaylist.thumbnailUrl).to.exist
      expect(videoPlaylist.embedUrl).to.equal('https://boomboom2.cpy.re/video-playlists/embed/fgei1ws1oa6FCaJ2qZPG29')
      ...
      expect(videoPlaylist.ownerAccount.url).to.equal('https://boomboom2.cpy.re/accounts/chocobozzz')
      expect(videoPlaylist.ownerAccount.name).to.equal('chocobozzz')
      expect(videoPlaylist.ownerAccount.host).to.equal('boomboom2.cpy.re')
      expect(videoPlaylist.ownerAccount.avatars.length).to.equal(4, 'Account should have four avatar images')

      expect(videoPlaylist.videoChannel.url).to.equal('https://boomboom2.cpy.re/video-channels/chocobozzz_channel')
      expect(videoPlaylist.videoChannel.name).to.equal('chocobozzz_channel')
      expect(videoPlaylist.videoChannel.host).to.equal('boomboom2.cpy.re')
    ...
    it('Should make host search and have appropriate results', async function () {
      ...
      await check({ search: 'E2E playlist', host: 'boomboom2.cpy.re', sort: '-match' }, true)
    })
