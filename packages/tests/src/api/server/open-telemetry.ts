...
import { HttpStatusCode, VideoPrivacy, VideoResolution } from '@boomboom/boomboom-models'
import {
  cleanupTests,
  createSingleServer,
  makeRawRequest,
...
describe('Open Telemetry', function () {
  ...
  describe('Metrics', function () {
    ...
    it('Should enable open telemetry metrics', async function () {
      ...
      expect(res.text).to.contain('boomboom_job_queue_total{')
    ...
    it('Should have runner observer metrics', async function () {
      ...
      try {
        ...
        expect(res.text).to.match(/boomboom_runner_count\{[^}]*\} 1(\.0)?/)
        expect(res.text).to.match(
          new RegExp(`boomboom_runner_info\\{[^}]*runnerName="${runnerName}"[^}]*version="${runnerVersion}"[^}]*\\} 1(\\.0)?`)
        )

        const secondsMatch = res.text.match(
          new RegExp(`boomboom_runner_seconds_since_last_contact\\{[^}]*runnerName="${runnerName}"[^}]*\\} ([0-9]+(?:\\.[0-9]+)?)`)
        )
    ...
    it('Should have playback metrics', async function () {
      ...
      expect(res.text).to.contain('boomboom_playback_http_downloaded_bytes_total{')
      expect(res.text).to.contain('boomboom_playback_p2p_peers{')
    ...
    it('Should take the last playback metric', async function () {
      ...
      expect(res.text).to.contain(`boomboom_playback_p2p_peers${label} 42`)
      expect(res.text).to.not.contain(`boomboom_playback_p2p_peers${label} 7`)
    })
