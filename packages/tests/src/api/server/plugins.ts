...
import { HttpStatusCode, PluginType } from '@boomboom/boomboom-models'
import {
  cleanupTests,
  createSingleServer,
  killallServers,
...
describe('Test plugins', function () {
  ...
  it('Should list and search available plugins and themes', async function () {
...

    {
      const body = await command.listAvailable({
        ...
        pluginType: PluginType.THEME,
        search: 'background-red',
        currentBoomBoomEngine: '1.0.0'
      })

      const p = body.data.find(p => p.npmName === 'boomboom-theme-background-red')
  ...
  it('Should install a plugin and a theme', async function () {
    ...
    await command.install({ npmName: 'boomboom-plugin-hello-world' })
    await command.install({ npmName: 'boomboom-theme-background-red' })
  })
  ...
  it('Should have the plugin loaded in the configuration', async function () {
    for (const config of [ await server.config.getConfig(), await server.config.getIndexHTMLConfig() ]) {
      ...
      expect(theme.npmName).to.equal('boomboom-theme-background-red')

      const plugin = config.plugin.registered.find(r => r.name === 'hello-world')
      expect(plugin).to.not.be.undefined
      expect(plugin.npmName).to.equal('boomboom-plugin-hello-world')
    }
  })
  ...
  it('Should get public settings', async function () {
    const body = await command.getPublicSettings({ npmName: 'boomboom-plugin-hello-world' })
  ...
  it('Should update the settings', async function () {
    ...
    await command.updateSettings({
      npmName: 'boomboom-plugin-hello-world',
      settings
    })
  })
  ...
  it('Should get a plugin and a theme', async function () {
    {
      const plugin = await command.get({ npmName: 'boomboom-plugin-hello-world' })
      ...
      expect(plugin.boomboomEngine).to.exist
    ...
    {
      const plugin = await command.get({ npmName: 'boomboom-theme-background-red' })
  ...
  it('Should update the plugin and the theme', async function () {
    ...
    async function testUpdate (type: 'plugin' | 'theme', name: string) {
      ...
      const packageJSON = await command.getPackageJSON(`boomboom-${type}-${name}`)
      const oldVersion = packageJSON.version

      packageJSON.version = '0.0.1'
      await command.updatePackageJSON(`boomboom-${type}-${name}`, packageJSON)
      ...
      {
        await command.update({ npmName: `boomboom-${type}-${name}` })

        const plugin = await getPluginFromAPI()
        expect(plugin.version).to.equal(oldVersion)

        const updatedPackageJSON = await command.getPackageJSON(`boomboom-${type}-${name}`)
  ...
  it('Should uninstall the plugin', async function () {
    await command.uninstall({ npmName: 'boomboom-plugin-hello-world' })
  ...
  it('Should uninstall the theme', async function () {
    await command.uninstall({ npmName: 'boomboom-theme-background-red' })
  })
  ...
  it('Should return default value for a setting if not set in DB', async function () {
    ...
    try {
      ...
      await command.uninstall({ npmName: 'boomboom-plugin-test' })
    }
  })
