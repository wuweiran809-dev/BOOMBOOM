...
import { ActorFollow, FollowState } from '@boomboom/boomboom-models'
import {
  cleanupTests,
...
async function checkServer1And2HasFollowers (servers: BoomBoomServer[], state = 'accepted') {
  const fns = [
  ...
  for (const fn of fns) {
    ...
    expect(follow.follower.url).to.equal(servers[0].url + '/accounts/boomboom')
    expect(follow.following.url).to.equal(servers[1].url + '/accounts/boomboom')
  }
}

async function checkFollows (options: {
  follower: BoomBoomServer
  followerState: FollowState | 'deleted'

  following: BoomBoomServer
  followingState: FollowState | 'deleted'
}) {
  const { follower, followerState, followingState, following } = options

  const followerUrl = follower.url + '/accounts/boomboom'
  const followingUrl = following.url + '/accounts/boomboom'
...
async function checkNoFollowers (servers: BoomBoomServer[]) {
  const fns = [
    servers[0].follows.getFollowings.bind(servers[0].follows),
    servers[1].follows.getFollowers.bind(servers[1].follows)
  ]
...
describe('Test follows moderation', function () {
  ...
  describe('Manual approbation for the instance', function () {
    ...
    it('Should accept a follower', async function () {
      await commands[1].acceptFollower({ follower: 'boomboom@' + servers[0].host })
      await waitJobs(servers)
    ...
    it('Should reject another follower', async function () {
      ...
      await commands[2].rejectFollower({ follower: 'boomboom@' + servers[0].host })
      await waitJobs(servers)
  ...
  describe('Accept/reject state', function () {
    ...
    it('Should be able to reject a previously accepted follower', async function () {
      await commands[1].rejectFollower({ follower: 'boomboom@' + servers[0].host })
      await waitJobs(servers)
