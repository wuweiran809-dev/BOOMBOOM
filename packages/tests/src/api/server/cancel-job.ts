...
import { Job, JobState, JobType, VideoDetails, VideoPrivacy, VideoStudioTask } from '@boomboom/boomboom-models'
...
async function waitForActiveJob (server: BoomBoomServer, uuid: string, jobType: JobType) {
  let activeJob: Job
...
async function waitForJobCancellation (server: BoomBoomServer, uuid: string, jobType: JobType) {
...
async function findActiveJobByUUID (server: BoomBoomServer, uuid: string, jobType: JobType) {
