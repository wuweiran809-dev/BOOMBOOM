...
import { ActorImageType, CustomConfig, HttpStatusCode, LogoType, VideoCommentPolicy, VideoPrivacy } from '@boomboom/boomboom-models'
import {
  BoomBoomServer,
  cleanupTests,
  createSingleServer,
...
function checkInitialConfig (server: BoomBoomServer, data: CustomConfig) {
  expect(data.instance.name).to.equal('BoomBoom')
  expect(data.instance.shortDescription).to.equal(
    'BoomBoom, an ActivityPub-federated video streaming platform using P2P directly in your web browser.'
  )
  expect(data.instance.description).to.equal('Welcome to this BoomBoom instance!')
...
function buildNewCustomConfig (server: BoomBoomServer): CustomConfig {
  return {
    instance: {
      name: 'BoomBoom updated',
      ...
      defaultLanguage: 'fr',
      social: {
        externalLink: 'https://joinboomboom.org/',
        mastodonLink: 'https://framapiaf.org/@boomboom',
        blueskyLink: 'https://bsky.app/profile/joinboomboom.org',
        xLink: 'https://x.org/@joinboomboom'
      },
...
describe('Test config', function () {
  ...
  describe('Config keys', function () {
    ...
    it('Should fetch the about information', async function () {
      ...
      expect(instance.name).to.equal('BoomBoom updated')
    ...
    it('Should enable/disable security headers', async function () {
...

      {
        ...
        expect(res.headers['x-powered-by']).to.equal('BoomBoom')
      }
  ...
  describe('Image files', function () {
    ...
    describe('Avatar', function () {
      ...
      it('Should have the avatars in the AP representation of the instance', async function () {
        const res = await makeActivityPubGetRequest(server.url, '/accounts/boomboom')
      ...
      describe('SVG logos', async function () {
        it('Should upload SVG on compatible endpoints', async function () {
          for (const type of [ 'favicon', 'header-wide', 'header-square', 'opengraph' ] as LogoType[]) {
            await server.config.updateInstanceLogo({ type, fixture: 'boomboom.svg' })
  ...
  describe('Manifest', function () {
    ...
    it('Should generate the manifest file without avatar', async function () {
      ...
      expect(body.name).to.equal('BoomBoom manifest')
