...
import { cleanupTests, createMultipleServers, BoomBoomServer, setAccessTokensToServers, waitJobs } from '@boomboom/boomboom-server-commands'

async function checkFollow (follower: BoomBoomServer, following: BoomBoomServer, exists: boolean) {
  {
...
async function server1Follows2 (servers: BoomBoomServer[]) {
  await servers[0].follows.follow({ hosts: [ servers[1].host ] })

  await waitJobs(servers)
}

async function resetFollows (servers: BoomBoomServer[]) {
  try {
...
describe('Test auto follows', function () {
  ...
  describe('Auto follow back', function () {
    ...
    it('Should wait the acceptation before auto follow back', async function () {
      ...
      await servers[1].follows.acceptFollower({ follower: 'boomboom@' + servers[0].host })
      await waitJobs(servers)
