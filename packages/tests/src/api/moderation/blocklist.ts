...
async function checkAllVideos (server: BoomBoomServer, token: string) {
  {
...
async function checkAllComments (server: BoomBoomServer, token: string, videoUUID: string) {
...
async function checkCommentNotification (
  mainServer: BoomBoomServer,
  comment: { server: BoomBoomServer, token: string, videoUUID: string, text: string },
      ...
      it('Should list blocked accounts', async function () {
        {
          ...
          const block = body.data[0]
          expect(block.byAccount.displayName).to.equal('boomboom')
          expect(block.byAccount.name).to.equal('boomboom')
          expect(block.blockedAccount.displayName).to.equal('user2')
          expect(block.blockedAccount.name).to.equal('user2')
          expect(block.blockedAccount.host).to.equal('' + servers[1].host)
        }
