...
import { HttpStatusCode, BoomBoomPlugin, PluginType } from '@boomboom/boomboom-models'
import {
  cleanupTests,
  createSingleServer,
  makeGetRequest,
...
describe('Test server plugins API validators', function () {
  ...
  const npmPlugin = 'boomboom-plugin-hello-world'
  const pluginName = 'hello-world'
  let npmVersion: string

  const themePlugin = 'boomboom-theme-background-red'
  ...
  before(async function () {
...

    {
      ...
      const plugin = res.body as BoomBoomPlugin
  ...
  describe('When listing available plugins/themes', function () {
    ...
    it('Should fail with an invalid current boomboom engine', async function () {
      const query = { ...baseQuery, currentBoomBoomEngine: '1.0' }
  ...
  describe('When getting a plugin or the registered settings or public settings', function () {
    ...
    it('Should fail with an invalid npm name', async function () {
      ...
      for (const suffix of [ 'boomboom-plugin-TOTO', 'boomboom-plugin-TOTO/registered-settings', 'boomboom-plugin-TOTO/public-settings' ]) {
        await makeGetRequest({
          url: server.url,
          path: path + suffix,
          token: server.accessToken,
  ...
  describe('When updating plugin settings', function () {
    ...
    it('Should fail with an invalid npm name', async function () {
      ...
      await makePutBodyRequest({
        url: server.url,
        path: path + 'boomboom-plugin-TOTO/settings',
        fields: { settings },
        token: server.accessToken,
        expectedStatus: HttpStatusCode.BAD_REQUEST_400
      })
    ...
    it('Should fail with an invalid npm name', async function () {
      ...
      for (const suffix of [ 'install', 'update', 'uninstall' ]) {
        await makePostBodyRequest({
          url: server.url,
          path: path + suffix,
          fields: { npmName: 'boomboom-plugin-TOTO' },
          token: server.accessToken,
          expectedStatus: HttpStatusCode.BAD_REQUEST_400
        })
      }
