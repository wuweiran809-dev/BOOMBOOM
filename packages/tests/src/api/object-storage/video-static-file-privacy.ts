...
import { getAllFiles, getHLS } from '@boomboom/boomboom-core-utils'
import { HttpStatusCode, LiveVideo, VideoDetails, VideoPrivacy, VideoResolution } from '@boomboom/boomboom-models'
import { areScalewayObjectStorageTestsDisabled } from '@boomboom/boomboom-node-utils'
import {
  cleanupTests,
  createSingleServer,
...
describe('Object storage for video static file privacy', function () {
  ...
  describe('VOD', function () {
    ...
    const correctPasswordHeader = { 'x-boomboom-video-password': correctPassword }
    const incorrectPasswordHeader = { 'x-boomboom-video-password': correctPassword + 'toto' }
    ...
    async function checkLiveFiles (live: LiveVideo, liveId: string, videoPassword?: string) {
      ...
      for (const url of [ hls.playlistUrl, hls.segmentsSha256Url ]) {
        ...
        if (videoPassword) {
          await makeRawRequest({
            url,
            headers: { 'x-boomboom-video-password': videoPassword },
            expectedStatus: HttpStatusCode.OK_200
          })

          await makeRawRequest({
            url,
            headers: { 'x-boomboom-video-password': 'incorrectPassword' },
            expectedStatus: HttpStatusCode.FORBIDDEN_403
          })
        }
      }
