...
describe('Test plugin CLI', function () {
  ...
  it('Should install a theme from stateless CLI', async function () {
    ...
    await server.cli.execWithEnv(`npm run plugin:install -- --npm-name boomboom-theme-background-red`)
  })

  it('Should have the theme and the plugin registered when we restart boomboom', async function () {
    this.timeout(30000)
  ...
  it('Should uninstall a plugin from stateless CLI', async function () {
    ...
    await server.cli.execWithEnv(`npm run plugin:uninstall -- --npm-name boomboom-plugin-test`)
  })

  it('Should have removed the plugin on another boomboom restart', async function () {
    this.timeout(30000)
