...
import { YoutubeDLCLI } from '@boomboom/boomboom-server/core/helpers/youtube-dl/youtube-dl-cli.js'
import { logger } from '@boomboom/boomboom-server/core/helpers/logger.js'
import { CONFIG } from '@boomboom/boomboom-server/core/initializers/config.js'
...
describe('YoutubeDLCLI', function () {
  ...
  describe('wrapWithCookiesOptions', function () {
    ...
    it('Should prepend cookies file when configured and the file exists', async function () {
      ...
      const tempDir = await mkdtemp(join(tmpdir(), 'boomboom-cookies-'))
