...
function postCommand (server: BoomBoomServer, command: string, bodyArg?: object) {
  const body = { command }
...
describe('Test plugin helpers', function () {
  ...
  describe('Logger', function () {
    it('Should have logged things', async function () {
      await servers[0].servers.waitUntilLog(servers[0].host + ' boomboom-plugin-test-four', 1, false)
  ...
  describe('Config', function () {
    ...
    it('Should have the correct config', async function () {
      ...
      expect(res.body.serverConfig.instance.name).to.equal('BoomBoom')
    })
  })

  describe('Server', function () {
    it('Should get the server actor', async function () {
      await servers[0].servers.waitUntilLog('server actor name is boomboom')
    })
  })
