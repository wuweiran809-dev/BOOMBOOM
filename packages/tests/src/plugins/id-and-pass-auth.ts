...
import { HttpStatusCode, UserRole } from '@boomboom/boomboom-models'
import {
  cleanupTests,
  createSingleServer,
  BoomBoomServer,
...
describe('Test id and pass auth plugins', function () {
  ...
  it('Should display the correct configuration', async function () {
    ...
    expect(crashAuth.npmName).to.equal('boomboom-plugin-test-id-pass-auth-one')
  ...
  it('Should unregister spyro-auth and do not login existing Spyro', async function () {
    await server.plugins.updateSettings({
      npmName: 'boomboom-plugin-test-id-pass-auth-one',
  ...
  it('Should uninstall the plugin one and do not login existing Crash', async function () {
    await server.plugins.uninstall({ npmName: 'boomboom-plugin-test-id-pass-auth-one' })
  ...
  it('Should display plugin auth information in users list', async function () {
    ...
    expect(crash.pluginAuth).to.equal('boomboom-plugin-test-id-pass-auth-one')
    expect(laguna.pluginAuth).to.equal('boomboom-plugin-test-id-pass-auth-two')
  })
