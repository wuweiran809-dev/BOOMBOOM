...
import { getAudioStream, getVideoStream, getVideoStreamFPS, hasAudioStream, hasVideoStream } from '@boomboom/boomboom-ffmpeg'
import { VideoPrivacy, VideoResolution } from '@boomboom/boomboom-models'
import {
  cleanupTests,
  createSingleServer,
  BoomBoomServer,
...
async function createLiveWrapper (server: BoomBoomServer) {
...
function updateConf (server: BoomBoomServer, vodProfile: string, liveProfile: string) {
  ...
  describe('When using a plugin adding profiles to existing encoders', function () {
    ...
    describe('Live', function () {
      ...
      it('Should default to the default profile if the specified profile does not exist', async function () {
        ...
        await server.plugins.uninstall({ npmName: 'boomboom-plugin-test-transcoding-one' })
