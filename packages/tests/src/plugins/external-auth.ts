...
import { HttpStatusCode, HttpStatusCodeType, UserAdminFlag, UserRole } from '@boomboom/boomboom-models'
import {
  cleanupTests,
  createSingleServer,
  decodeQueryString,
...
describe('Test external auth plugins', function () {
  ...
  it('Should display the correct configuration', async function () {
    ...
    expect(auth2.npmName).to.equal('boomboom-plugin-test-external-auth-one')
  })
  ...
  it('Should unregister external-auth-2 and do not login existing Kefka', async function () {
    await server.plugins.updateSettings({
      npmName: 'boomboom-plugin-test-external-auth-one',
  ...
  it('Should uninstall the plugin one and do not login Cyan', async function () {
    await server.plugins.uninstall({ npmName: 'boomboom-plugin-test-external-auth-one' })
