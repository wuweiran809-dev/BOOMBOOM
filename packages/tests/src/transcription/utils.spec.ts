...
describe('downloadFile', function () {
  const testDirectory = join(tmpdir(), 'boomboom-transcription', 'utils')
  ...
  it(`Downloads a file and write it to the disk `, async function () {
    const filePath = await downloadFile('https://download.cpy.re/boomboom/4k_file.txt', testDirectory)
