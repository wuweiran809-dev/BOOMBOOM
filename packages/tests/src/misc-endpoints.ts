...
import { HttpStatusCode, VideoPrivacy } from '@boomboom/boomboom-models'
...
describe('Test misc endpoints', function () {
  ...
  describe('Test a well known endpoints', function () {
    ...
    it('Should get nodeinfo', async function () {
      ...
      expect(res.body.links[1].href).to.equal(server.url + '/accounts/boomboom')
    })
    ...
    it('Should test webfinger', async function () {
      const resource = 'acct:boomboom@' + server.host
      const accountUrl = server.url + '/accounts/boomboom'
  ...
  describe('Test classic static endpoints', function () {
    ...
    it('Should get nodeinfo', async function () {
      ...
      expect(res.body.software.name).to.equal('boomboom')
