...
async function check (server: BoomBoomServer, videoUUID: string, exists = true) {
...
describe('Official plugin auto-block videos', function () {
  ...
  before(async function () {
    ...
    for (const server of servers) {
      await server.plugins.install({ npmName: 'boomboom-plugin-auto-block-videos' })
    }
  ...
  it('Should update plugin settings', async function () {
    await servers[0].plugins.updateSettings({
      npmName: 'boomboom-plugin-auto-block-videos',
      settings: {
