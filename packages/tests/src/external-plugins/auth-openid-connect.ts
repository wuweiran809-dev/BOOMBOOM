...
import { HttpStatusCode, HttpStatusCodeType, UserRole } from '@boomboom/boomboom-models'
import {
  cleanupTests,
  createSingleServer,
  makeRawRequest,
...
describe('Official plugin auth-openid-connect', function () {
  ...
  before(async function () {
    ...
    await server.plugins.install({ npmName: 'boomboom-plugin-auth-openid-connect' })
  })
  ...
  it('Should login with the appropriate username/password', async function () {
    const boomboomRes = await getOpenIdUrl(openIdLoginUrl)
    const kcRes = await loginOnKeycloak({ loginPageUrl: extractLocation(boomboomRes) })

    const ptBypassPath = await sendBackKeycloakCode({ boomboomRes, kcRes, success: true })
  ...
  it('Should allow to redirect on the app by default', async function () {
    await getOpenIdUrl(openIdLoginUrl, 'boomboom://joinboomboom.org/mysuperlogin', HttpStatusCode.FOUND_302)
    await getOpenIdUrl(openIdLoginUrl, 'boomboom://joinboomboom.org/', HttpStatusCode.FOUND_302)
    await getOpenIdUrl(openIdLoginUrl, 'peertdube://joinboomboom.org/', HttpStatusCode.FORBIDDEN_403)
    await getOpenIdUrl(openIdLoginUrl, 'aboomboom://joinboomboom.org/', HttpStatusCode.FORBIDDEN_403)
    await getOpenIdUrl(openIdLoginUrl, 'boomboom://joinboomboom/', HttpStatusCode.FORBIDDEN_403)
  })
  ...
  it('Should correctly redirect the user', async function () {
    const boomboomRes = await getOpenIdUrl(openIdLoginUrl, 'http://example.com')
    const kcRes = await loginOnKeycloak({ loginPageUrl: extractLocation(boomboomRes) })

    const ptBypassPath = await sendBackKeycloakCode({ boomboomRes, kcRes, redirectUrl: 'http://example.com', success: true })
    const externalAuthToken = new URL(ptBypassPath, server.url).searchParams.get('externalAuthToken')
  ...
  it('Should only login allowed groups', async function () {
    {
      ...
      const boomboomRes = await getOpenIdUrl(openIdLoginUrl)
      const kcRes = await loginOnKeycloak({ loginPageUrl: extractLocation(boomboomRes), username: 'user_group1' })
      const redirectUrl = await sendBackKeycloakCode({ boomboomRes, kcRes, success: false })
...

    {
      ...
      const boomboomRes = await getOpenIdUrl(openIdLoginUrl)
      const kcRes = await loginOnKeycloak({ loginPageUrl: extractLocation(boomboomRes), username: 'user_group1' })
      await sendBackKeycloakCode({ boomboomRes, kcRes, username: 'user_group1_example.com', success: true })
    }
  ...
  it('Should correctly map the user role property', async function () {
    ...
    {
      const boomboomRes = await getOpenIdUrl(openIdLoginUrl)
      const kcRes = await loginOnKeycloak({ loginPageUrl: extractLocation(boomboomRes), username: 'moderator' })
      const ptBypassPath = await sendBackKeycloakCode({ boomboomRes, kcRes, username: 'moderator_example.com', success: true })
    ...
    {
      const boomboomRes = await getOpenIdUrl(openIdLoginUrl)
      const kcRes = await loginOnKeycloak({ loginPageUrl: extractLocation(boomboomRes), username: 'user_group2' })
      const ptBypassPath = await sendBackKeycloakCode({ boomboomRes, kcRes, username: 'user_group2_example.com', success: true })
...
async function getOpenIdUrl (
  ...
  const boomboomRes = await makeRawRequest({
    url: openIdLoginUrl,
    query,
    expectedStatus
  })

  const kcLocation = boomboomRes.headers['location']
...
async function sendBackKeycloakCode (options: {
  boomboomRes: Response
  kcRes: Response
  success: boolean
  redirectUrl?: string
  username?: string
}) {
  const { boomboomRes, kcRes, redirectUrl = '/login', success, username = 'myuser_example.com' } = options
  ...
  const res = await makeRawRequest({
    ...
    method: 'POST',
    headers: {
      'Cookie': extractCookies(boomboomRes),
...
async function waitForAuthReady (server: BoomBoomServer) {
...
async function updatePluginSettings (server: BoomBoomServer, override?: Record<string, any>) {
  ...
  await server.plugins.updateSettings({
    npmName: 'boomboom-plugin-auth-openid-connect',
    settings: {
      ...pluginSettings,
