...
import { PluginType_Type, UserNotification, UserNotificationType } from '@boomboom/boomboom-models'
import { expect } from 'chai'
import { CheckerBaseParams, CheckerType, checkNotification } from './shared/notification-checker.js'

export async function checkNewBoomBoomVersion (
  ...
  const notificationType = UserNotificationType.NEW_BOOMBOOM_VERSION
  ...
  function notificationChecker (notification: UserNotification, checkType: CheckerType) {
    if (checkType === 'presence') {
      ...
      expect(notification.boomboom).to.exist
      expect(notification.boomboom.latestVersion).to.equal(latestVersion)
    } else {
      expect(notification).to.satisfy((n: UserNotification) => {
        return n?.boomboom?.latestVersion !== latestVersion
      })
    }
  }
