...
import { UserNotification, UserNotificationType } from '@boomboom/boomboom-models'
...
export async function checkNewInstanceFollower (
  ...
  function notificationChecker (notification: UserNotification, checkType: CheckerType) {
    if (checkType === 'presence') {
      ...
      expect(notification.actorFollow.follower.name).to.equal('boomboom')
      expect(notification.actorFollow.follower.host).to.equal(followerHost)

      expect(notification.actorFollow.following.name).to.equal('boomboom')
    } else {
      expect(notification).to.satisfy(n => {
  ...
  function emailNotificationFinder (email: object) {
    ...
    return text.includes('BoomBoom has a new follower') && text.includes(followerHost)
  }
...
export async function checkAutoInstanceFollowing (
  ...
  function notificationChecker (notification: UserNotification, checkType: CheckerType) {
    if (checkType === 'presence') {
      ...
      expect(following.name).to.equal('boomboom')
