...
import { getFilenameFromUrl } from '@boomboom/boomboom-node-utils'
import {
  ConfigCommand,
...
export async function downloadZIP (server: BoomBoomServer, userId: number) {
...
export async function probeZIPFile (zip: JSZip, path: string, base = '/') {
  ...
  const basePath = join(tmpdir(), 'boomboom-test')
