import { omit, pick } from '@boomboom/boomboom-core-utils'
import {
  Account,
  ActorImageType,
  HTMLServerConfig,
...
export function checkIndexTags (html: string, title: string, description: string, css: string, config: ServerConfig) {
  ...
  expect(html).to.contain(`<script type="application/javascript">window.BoomBoomServerConfig = ${configEscapedString}</script>`)
}
