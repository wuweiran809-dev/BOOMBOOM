import { millisecondsToTime } from '@boomboom/boomboom-core-utils'
import { buildAbsoluteFixturePath, buildSUUID } from '@boomboom/boomboom-node-utils'
import {
  TranscriptFile,
  TranscriptionEngine,
  ...
  TranscriptionModel,
  transcriberFactory
} from '@boomboom/boomboom-transcription'
...
void (async () => {
  ...
  const transcriptDirectory = join(tmpdir(), 'boomboom-transcription', 'benchmark')
  const pipDirectory = join(tmpdir(), 'boomboom-transcription', 'pip')
