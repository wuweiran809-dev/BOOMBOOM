...
import { VideoDetails, VideoInclude, VideoPrivacy } from '@boomboom/boomboom-models'
...
export async function waitUntilLivePublishedOnAllServers (servers: BoomBoomServer[], videoId: string) {
  for (const server of servers) {
    await server.live.waitUntilPublished({ videoId })
  }
}

export async function waitUntilLiveWaitingOnAllServers (servers: BoomBoomServer[], videoId: string) {
  for (const server of servers) {
    await server.live.waitUntilWaiting({ videoId })
  }
}

export async function waitUntilLiveReplacedByReplayOnAllServers (servers: BoomBoomServer[], videoId: string) {
  for (const server of servers) {
    await server.live.waitUntilReplacedByReplay({ videoId })
  }
}

export async function findExternalSavedVideo (server: BoomBoomServer, liveVideoUUID: string) {
