...
import { getAllPrivacies, omit, pick, wait } from '@boomboom/boomboom-core-utils'
import {
  HttpStatusCode,
  HttpStatusCodeType,
  ResultList,
...
import { buildAbsoluteFixturePath, buildUUID } from '@boomboom/boomboom-node-utils'
...
export class VideosCommand extends AbstractCommand {
  ...
  getWithPassword (
    ...
    return this.getRequestBody<VideoDetails>({
      ...options,
      headers: {
        'x-boomboom-video-password': options.password
      },
      path,
      implicitToken: false,
      defaultExpectedStatus: HttpStatusCode.OK_200
