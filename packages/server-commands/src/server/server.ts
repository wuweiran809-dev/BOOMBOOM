...
import { User, Video, VideoChannelSync, VideoCreateResult, VideoDetails } from '@boomboom/boomboom-models'
import { parallelTests, root } from '@boomboom/boomboom-node-utils'
...
export class BoomBoomServer {
  ...
  async run (configOverrideArg?: any, options: RunServerOptions = {}) {
    ...
    const key = 'Database boomboom_test' + this.internalServerNumber + ' is ready'
    ...
    const boomboomArgs = options.boomboomArgs || []

    return new Promise<void>((res, rej) => {
      const self = this
      let aggregatedLogs = ''

      this.app = fork(join(root(), 'dist', 'server.js'), boomboomArgs, forkOptions)

      const onBoomBoomExit = () => rej(new Error('Process exited:\n' + aggregatedLogs))
      const onParentExit = () => {
      ...
      this.app.on('exit', onBoomBoomExit)
      ...
      this.app.stdout.on('data', function onStdout (data) {
        ...
        if (options.hideLogs === false) {
          ...
          self.app.removeListener('exit', onBoomBoomExit)
        }
