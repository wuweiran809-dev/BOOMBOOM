...
import { isGithubCI, root } from '@boomboom/boomboom-node-utils'
...
export class ServersCommand extends AbstractCommand {
  ...
  cleanupTests () {
    ...
    const saveGithubLogsIfNeeded = async () => {
      ...
      const origin = this.buildDirectory('logs/boomboom.log')
      const destname = `boomboom-${this.server.internalServerNumber}.log`
    ...
    const saveDBIfNeeded = async () => {
      ...
      await ensureDir('artifacts')
      const destname = join('artifacts', `boomboom-${this.server.internalServerNumber}.sql`)
      console.log('Saving database %s.', destname)

      exec(`pg_dump boomboom_test${this.server.internalServerNumber} > ${destname}`)
    }
  ...
  async waitUntilLog (str: string, count = 1, strictCount = true) {
    const logfile = this.buildDirectory('logs/boomboom.log')
  ...
  getLogContent () {
    return readFile(this.buildDirectory('logs/boomboom.log'))
  }
}
