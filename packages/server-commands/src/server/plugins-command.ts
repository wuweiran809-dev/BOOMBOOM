...
export class PluginsCommand extends AbstractCommand {
  static getPluginTestPath (suffix = '') {
    return buildAbsoluteFixturePath('boomboom-plugin-test' + suffix)
  }
  ...
  list (
    ...
    return this.getRequestBody<ResultList<BoomBoomPlugin>>({
      ...options,
  ...
  listAvailable (
    ...
    const { start, count, sort, pluginType, search, currentBoomBoomEngine } = options
    const path = '/api/v1/plugins/available'

    const query: BoomboomPluginIndexList = {
      start,
      count,
      sort,
      pluginType,
      currentBoomBoomEngine,
      search
    }

    return this.getRequestBody<ResultList<BoomBoomPluginIndex>>({
      ...options,
  ...
  get (
    ...
    return this.getRequestBody<BoomBoomPlugin>({
      ...options,
