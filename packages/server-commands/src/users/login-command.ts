...
import { HttpStatusCode, BoomBoomProblemDocument, ResultList, TokenSession } from '@boomboom/boomboom-models'
...
export class LoginCommand extends AbstractCommand {
  ...
  private _login (options: LoginOptions) {
    ...
    if (otpToken) headers['x-boomboom-otp'] = otpToken
  ...
  private unwrapLoginBody (body: any) {
    return body as { access_token: string, refresh_token: string } & BoomBoomProblemDocument
  }
