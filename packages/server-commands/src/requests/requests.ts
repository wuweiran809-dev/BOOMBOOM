...
import { pick, queryParamsToObject } from '@boomboom/boomboom-core-utils'
    ...
    if (options.expectedStatus && res.status !== options.expectedStatus) {
      const err = new Error(
        `Expected status ${options.expectedStatus}, got ${res.status}. ` +
          ...
          'https://github.com/Chocobozzz/BoomBoom/blob/develop/support/doc/development/tests.md#debug-server-logs'
      )
      ;(err as any).res = res
