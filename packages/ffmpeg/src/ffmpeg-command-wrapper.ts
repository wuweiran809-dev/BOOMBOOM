import { arrayify, pick, promisify0 } from '@boomboom/boomboom-core-utils'
import {
  AvailableEncoders,
...
export class FFmpegCommandWrapper {
  ...
  async getEncoderBuilderResult (
    ...
    for (const encoder of encodersToTry) {
      ...
      if (!encoders[encoder]) {
        this.logger.debug(`Encoder ${encoder} not available in boomboom encoders, skipping.`, this.lTags)
        continue
      }
  ...
  private async checkFFmpegEncoders (boomboomAvailableEncoders: AvailableEncoders): Promise<Map<string, boolean>> {
    ...
    for (const type of [ 'live', 'vod' ]) {
      for (const streamType of [ 'audio', 'video' ]) {
        for (const encoder of boomboomAvailableEncoders.encodersToTry[type][streamType]) {
