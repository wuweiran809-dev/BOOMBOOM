...
**BoomBoom does not support webserver host change**. Keep in mind your domain
name is definitive after your first BoomBoom start.
...
```shell
cd /your/boomboom/directory
```

#### Get the latest Compose file

```shell
curl https://raw.githubusercontent.com/chocobozzz/BoomBoom/master/support/docker/production/docker-compose.yml > docker-compose.yml
```

View the source of the file you're about to download: [docker-compose.yml](https://github.com/Chocobozzz/BoomBoom/blob/master/support/docker/production/docker-compose.yml)

#### Get the latest env_file

```shell
curl https://raw.githubusercontent.com/Chocobozzz/BoomBoom/master/support/docker/production/.env > .env
```

View the source of the file you're about to download: [.env](https://github.com/Chocobozzz/BoomBoom/blob/master/support/docker/production/.env)
...
In the downloaded example [.env](https://github.com/Chocobozzz/BoomBoom/blob/master/support/docker/production/.env), you must replace:
...
[/support/docker/production/config/custom-environment-variables.yaml](https://github.com/Chocobozzz/BoomBoom/blob/master/support/docker/production/config/custom-environment-variables.yaml) and can be
intuited from usage.
...
The container will generate the configuration by replacing `${WEBSERVER_HOST}` and `${BOOMBOOM_HOST}` using your docker compose env file.

```shell
mkdir -p docker-volume/nginx docker-volume/nginx-logs
curl https://raw.githubusercontent.com/Chocobozzz/BoomBoom/master/support/nginx/boomboom > docker-volume/nginx/boomboom
```
...
You can change the automatically created password for user root by running this command from boomboom's root directory:
```shell
docker compose exec -u boomboom boomboom npm run reset-password -- -u root
```

You can also grep your boomboom container's logs for the default `root` password. You're going to want to run `docker-compose logs boomboom | grep -A1 root` to search the log output for your new BoomBoom's instance admin credentials which will look something like this.

```bash
docker compose logs boomboom | grep -A1 root

boomboom_1  | [example.com:443] 2019-11-16 04:26:06.082 info: Username: root
boomboom_1  | [example.com:443] 2019-11-16 04:26:06.083 info: User password: abcdefghijklmnop
```
...
boomboom._domainkey.mydomain.tld.	IN	TXT	( "v=DKIM1; h=sha256; k=rsa; "
	  "p=MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEA0Dx7wLGPFVaxVQ4TGym/eF89aQ8oMxS9v5BCc26Hij91t2Ci8Fl12DHNVqZoIPGm+9tTIoDVDFEFrlPhMOZl8i4jU9pcFjjaIISaV2+qTa8uV1j3MyByogG8pu4o5Ill7zaySYFsYB++cHJ9pjbFSC42dddCYMfuVgrBsLNrvEi3dLDMjJF5l92Uu8YeswFe26PuHX3Avr261n"
	  "j5joTnYwat4387VEUyGUnZ0aZxCERi+ndXv2/wMJ0tizq+a9+EgqIb+7lkUc2XciQPNuTujM25GhrQBEKznvHyPA6fHsFheymOuB763QpkmnQQLCxyLygAY9mE/5RY+5Q6J9oDOQIDAQAB" )  ; ----- DKIM key boomboom for mydomain.tld
```

#### Administrator password

See the production guide ["Administrator" section](https://docs.joinboomboom.org/install/any-os#administrator)

#### What now?

See the production guide ["What now" section](https://docs.joinboomboom.org/install/any-os#what-now).
...
Stop, delete the containers and internal volumes (to invalidate static client files shared by `boomboom` and `webserver` containers):
...
```shell
mv docker-volume/nginx/boomboom docker-volume/nginx/boomboom.bak
curl https://raw.githubusercontent.com/Chocobozzz/BoomBoom/master/support/nginx/boomboom > docker-volume/nginx/boomboom
```
...
When you're ready, go inside your BoomBoom docker compose directory:
...
```sh
mkdir -p backups
docker compose stop boomboom webserver certbot
```
...
Then, reset the BoomBoom database user password to fix a potential authentication issue if the old password hash algorithm has been deprecated.
...
```sh
docker compose up -d boomboom webserver certbot
docker compose logs -f boomboom
```
...
```shell
git clone https://github.com/chocobozzz/BoomBoom /tmp/boomboom
cd /tmp/boomboom
...
We don't have a Docker image for development. See [the CONTRIBUTING guide](https://docs.joinboomboom.org/contribute/getting-started#develop) for more information on how you can hack BoomBoom!
