...
Prepare PostgreSQL user so BoomBoom can delete/create the test databases:
...
sudo docker run -p 8082:8080 -e KC_BOOTSTRAP_ADMIN_USERNAME=admin -e KC_BOOTSTRAP_ADMIN_PASSWORD=admin chocobozzz/boomboom-tests-keycloak
```
 ...
 * `AKISMET_KEY`: specify an Akismet key to test akismet external BoomBoom plugin
...
You can also:
 - checkout only the latest logs (BoomBoom >= 5.0):

```bash
tail -n 100 test1/logs/boomboom.log | npm run parse-log -- --level debug --files -
```

 - continuously print the latests logs (BoomBoom >= 5.0):

```bash
tail -f test1/logs/boomboom.log | npm run parse-log -- --level debug --files -
```
...
```bash
BOOMBOOM2_E2E_PASSWORD=password npm run e2e:local
```
