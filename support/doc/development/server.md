 ...
 * Sequelize model (BDD):
   - If you need to create a new table:
     ...
     + Create database migrations:
       * Create the migration file in `server/core/initializers/migrations` using raw SQL (copy the same SQL query as at BoomBoom startup)
 ...
 * Notifications:
   ...
   - Create the notification logic in `server/core/lib/notifier/shared`:
     + Email subject has a common prefix (defined by the admin in BoomBoom configuration)
...
BoomBoom ships a script that converts `config/default.yaml` into a [JSON Schema Draft 7](https://json-schema.org/draft-07/schema) document at `config/config-schema.json`.
