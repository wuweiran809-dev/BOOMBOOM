...
Main dependencies supported by BoomBoom:

 * `node` LTS (**>= 22.12 and <25**)
 * `pnpm` >= 10.x
 * `postgres` >=10.x
 * `redis-server` >=6.2
 * `ffmpeg` >=4.3 (using a ffmpeg static build [is not recommended](https://github.com/Chocobozzz/BoomBoom/issues/6308))
 * `python` >=3.8
 * `pip`
...
Now that dependencies are installed, before running BoomBoom you should start PostgreSQL and Redis:
...
1. You need to use a more up to date version of G++ in order to run the `npm run install-node-dependencies` command, hence the installation of devtoolset-7.
    ...
    ```sh
    sudo -H -u boomboom CC=/opt/rh/devtoolset-7/root/usr/bin/gcc CXX=/opt/rh/devtoolset-7/root/usr/bin/g++ npm run install-node-dependencies -- --production
    ```
...
Now that dependencies are installed, before running BoomBoom you should enable and start PostgreSQL and Redis:
...
1. Unknown missing steps:
    ...
    - /var/www/boomboom does not exist yet (expected? done in future steps? documentation?).
...
1. Configure nginx
    ...
    sudo ln -s /etc/nginx/sites-enabled/boomboom /etc/nginx/conf.d/boomboom.conf
    ```
...
1. Firewall
    ...
    By default, you cannot access your server via public IP. To do so, you must configure the firewall.
      ...
      Ports used by boomboom dev setup:
    ```sh
...
1. bootstrap pkg, initialize db and install boomboom's dependencies, always as root (sudo not yet installed):
...
1. Run the services:
    ...
    If `sudo -u postgres createuser -P boomboom` gives you an `unknown user: postgres` error, you can try `sudo -u _postgres createuser -U boomboom`.

## Gentoo

1. Add this to ``/etc/portage/sets/boomboom``:
...
1. If you are on a "stable" Gentoo you need to accept the testing keyword ~amd64 pnpm:
    ...
    cat << EOF >> /etc/portage/package.keywords/boomboom
    # required by pnpm (argument) for BoomBoom
    sys-apps/pnpm ~amd64
    EOF
    ```

1. Compile the boomboom set:
