...
BoomBoom lets you embed videos and programmatically control their playback. This documentation covers how to interact with the BoomBoom Embed API.

## Playground

Any BoomBoom embed URL (ie `https://my-instance.example.com/videos/embed/52a10666-3a18-4e73-93da-e8d3c12c305a`) can be viewed as an embedding playground which
allows you to test various aspects of BoomBoom embeds. Simply replace `/embed` with `/test-embed` and visit the URL in a browser.
...
Given an existing BoomBoom embed `<iframe>` **with API enabled** (`https://my-instance.example.com/videos/embed/52a10666-3a18-4e73-93da-e8d3c12c305a?api=1`),
one can use the BoomBoom Embed API to control it by first including the library. You can include it via NPM with:

```
npm install @boomboom/embed-api
```

Now just use the `BoomBoomPlayer` class exported by the module:

```typescript
import { BoomBoomPlayer } from '@boomboom/embed-api.js'
...
```
<script src="https://unpkg.com/@boomboom/embed-api/build/player.min.js"></script>

<script>
  const BoomBoomPlayer = window['BoomBoomPlayer']
...
```typescript
let player = new BoomBoomPlayer(document.querySelector('iframe'))
...
You can customize BoomBoom player by specifying URL query parameters.
For example `https://my-instance.example.com/videos/embed/52a10666-3a18-4e73-93da-e8d3c12c305a?start=1s&stop=18s&loop=1&autoplay=1&muted=1&warningTitle=0&controlBar=0&boomboomLink=0&p2p=0`
...
It can be useful if you want to have a full control of the BoomBoom player.
...
Hide BoomBoom instance link in control bar.
...
See behaviour description [here](https://docs.joinboomboom.org/admin/configuration#vod-transcoding)
...
If the video requires a password, BoomBoom will wait a password provided by `setVideoPassword` method before loading the video.
...
### `getResolutions() : Promise<BoomBoomResolution[]>`

Get the available resolutions. A `BoomBoomResolution` looks like:
...
`duration` field and `ended` `playbackState` are available in BoomBoom >= 2.2.
