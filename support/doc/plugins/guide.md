 ...
 * Their name starts with `boomboom-theme-` instead of `boomboom-plugin-`
...
A plugin registers functions in JavaScript to execute when BoomBoom (server and client) fires events. There are 3 types of hooks:
 ...
 * `static`: same than `action` but BoomBoom waits their execution
...
All client scripts have scopes so BoomBoom client only loads scripts it needs:
...
```js
function register ({ registerHook, boomboomHelpers }) {
  registerHook({
    target: 'action:application.init',
    handler: () => onApplicationInit(boomboomHelpers)
  })
}
```

See the [plugin API reference](https://docs.joinboomboom.org/api/plugins) to see the complete hooks list.

### Static files

Plugins can declare static directories that BoomBoom will serve (images for example)
from `/plugins/{plugin-name}/{plugin-version}/static/`
or `/themes/{theme-name}/{theme-version}/static/` routes.

### CSS

Plugins can declare CSS files that BoomBoom will automatically inject in the client.
...
See the [CSS variables section](#css-variables) to also have details on how to easily theme BoomBoom.

### Server API (only for plugins)

#### Settings

Plugins can register settings, that BoomBoom will inject in the administration interface.
...
Plugins can store/load JSON data, that BoomBoom will store in its database (so don't put files in there).
...
function register ({
  ...
  const basePath = boomboomHelpers.plugin.getDataDirectoryPath()
...
function register ({
  ...
  router.get('/auth', async (req, res) => {
    const user = await boomboomHelpers.user.getAuthUser(res)
...
function register ({
  ...
  wss.on('connection', function connection(ws) {
    boomboomHelpers.logger.info('WebSocket connected!')
...
If you want to add a classic username/email and password auth method (like [LDAP](https://framagit.org/framasoft/boomboom/official-plugins/-/tree/master/boomboom-plugin-auth-ldap) for example):
...
function register (...) {
  ...
  registerIdAndPassAuth({
    ...
    // BoomBoom will try all id and pass plugins in the weight DESC order
    // Exposing this value in the plugin settings could be interesting
    getWeight: () => 60,

    // Optional function called by BoomBoom when the user clicked on the logout button
    onLogout: user => {
      console.log('User %s logged out.', user.username')
    },

    // Optional function called by BoomBoom when the access token or refresh token are generated/refreshed
    hookTokenValidity: ({ token, type }) => {
      if (type === 'access') return { valid: true }
      if (type === 'refresh') return { valid: false }
    },

    // Used by BoomBoom when the user tries to authenticate
    login: ({ id, password }) => {
...
You can also add an external auth method (like [OpenID](https://framagit.org/framasoft/boomboom/official-plugins/-/tree/master/boomboom-plugin-auth-openid-connect), [SAML2](https://framagit.org/framasoft/boomboom/official-plugins/-/tree/master/boomboom-plugin-auth-saml2) etc):
...
function register (...) {
  ...
  const result = registerExternalAuth({
    ...
    // If the user click on the auth button, BoomBoom will forward the request to this function
  ...
  router.use('/external-auth-callback', (req, res) => {
    // Forward the request to BoomBoom
    result.userAuthenticated({
      ...
      // https://github.com/Chocobozzz/BoomBoom/blob/develop/packages/models/src/users/user-flag.model.ts
      // BoomBoom >= 5.1
      adminFlags: 0,
      // Quota in bytes
      // BoomBoom >= 5.1
      ...
      // Ask BoomBoom to redirect on this URL instead of classic `/login` page
      // The URL will contain an `externalAuthToken` param that can be reused to authenticate to the BoomBoom REST API
      // Introduced in BoomBoom >= 7.3
      externalRedirectUri: 'https://mywebsite.example.com/boomboom-login-cb'
    })
  })
...
async function register ({
...

  {
    const builder = (options) => {
      ...
      // Always copy stream when BoomBoom use libfdk_aac or aac encoders
      return {
        copy: true
      }
    }
...
BoomBoom will try different encoders depending on their priority.
...
BoomBoom provides your plugin some helpers. For example:
...
async function register ({
  ...
  // Block a server
  {
    const serverActor = await boomboomHelpers.server.getServerActor()

    await boomboomHelpers.moderation.blockServer({ byAccountId: serverActor.Account.id, hostToBlock: '...' })
  }

  // Load a video
  {
    const video = await boomboomHelpers.videos.loadByUrl('...')
  }
}
```

See the [plugin API reference](https://docs.joinboomboom.org/api/plugins) to see the complete helpers list.

#### Federation

You can use some server hooks to federate plugin data to other BoomBoom instances that may have installed your plugin.
...
```js
function register (...) {
  const baseStaticUrl = boomboomHelpers.getBaseStaticRoute()
  const imageUrl = baseStaticUrl + '/images/chocobo.png'
}
```

And to get your plugin router route, use `boomboomHelpers.getBaseRouterRoute()`:
...
function register (...) {
  registerHook({
    ...
    handler: ({ video }) => {
      fetch(boomboomHelpers.getBaseRouterRoute() + '/my/plugin/api', {
        method: 'GET',
...
To notify the user with the BoomBoom ToastModule:

```js
function register (...) {
  const { notifier } = boomboomHelpers
...
```js
function register (...) {
  const { markdownRenderer } = boomboomHelpers
...
You can translate some strings of your plugin (BoomBoom will use your `translations` object of your `package.json` file):

```js
function register (...) {
  boomboomHelpers.translate('User name')
...
```js
function register (...) {
  boomboomHelpers.getServerConfig()
    .then(config => {
...
```js
async function register ({ registerVideoField, boomboomHelpers }) {
  const descriptionHTML = await boomboomHelpers.translate(descriptionSource)
  const commonOptions = {
    name: 'my-field-name,
...
BoomBoom will send this field value in `body.pluginData['my-field-name']` and fetch it from `video.pluginData['my-field-name']`.
...
BoomBoom provides some selectors (using `id` HTML attribute) on important blocks so plugins can easily change their style.

For example `#plugin-selector-login-form` could be used to hide the login form.

See the complete list on https://docs.joinboomboom.org/api/plugins

#### HTML placeholder elements

BoomBoom provides some HTML id so plugins can easily insert their own element:
...
BoomBoom can be easily themed using built-in CSS variables. The full list is available in [client/src/sass/include/_variables.scss](https://github.com/Chocobozzz/BoomBoom/blob/develop/client/src/sass/include/_variables.scss).

BoomBoom creates gradients of some CSS variables so you don't have to specify all variables yourself. For example, just specify `--bg-secondary` and BoomBoom will generate `--bg-secondary-450`, `--bg-secondary-400` and so on.

You can take inspiration from core BoomBoom themes in [client/src/sass/application.scss](https://github.com/Chocobozzz/BoomBoom/blob/develop/client/src/sass/application.scss) file:
...
See the [plugin API reference](https://docs.joinboomboom.org/api/plugins) to see the complete `doAction` list.

### Publishing

BoomBoom plugins and themes should be published on [NPM](https://www.npmjs.com/) so that BoomBoom indexes take into account your plugin (after ~ 1 day). An official plugin index is available on [packages.joinboomboom.org](https://packages.joinboomboom.org/api/v1/plugins), with no interface to present packages.

> The official plugin index source code is available at https://framagit.org/framasoft/boomboom/plugin-index
 ...
 * Add the appropriate prefix:
   * If you develop a plugin, add `boomboom-plugin-` prefix to your plugin name (for example: `boomboom-plugin-mysupername`)
   * If you develop a theme, add `boomboom-theme-` prefix to your theme name (for example: `boomboom-theme-mysupertheme`)
 * Clone the quickstart repository
 * Configure your repository
 * Update `README.md`
 * Update `package.json`
 * Register hooks, add CSS and static files
 * Test your plugin/theme with a local BoomBoom installation
 * Publish your plugin/theme on NPM

### Clone the quickstart repository

If you develop a plugin, clone the `boomboom-plugin-quickstart` repository:

```sh
git clone https://framagit.org/framasoft/boomboom/boomboom-plugin-quickstart.git boomboom-plugin-mysupername
```

If you develop a theme, clone the `boomboom-theme-quickstart` repository:

```sh
git clone https://framagit.org/framasoft/boomboom/boomboom-theme-quickstart.git boomboom-theme-mysupername
```
...
```sh
cd boomboom-plugin-mysupername # or cd boomboom-theme-mysupername
...
Update the `package.json` fields:
   * `name` (should start with `boomboom-plugin-` or `boomboom-theme-`)
   * `description`
   * `homepage`
   * `author`
   * `bugs`
   * `engine.boomboom` (the BoomBoom version compatibility, must be `>=x.y.z` and nothing else)

**Caution:** Don't update or remove other keys, or BoomBoom will not be able to index/install your plugin.
...
It's up to you to check the code you write will be compatible with the BoomBoom NodeJS version, and will be supported by web browsers.
...
The easiest way to use __Typescript__ for both front-end and backend code is to clone [boomboom-plugin-quickstart-typescript](https://github.com/JohnXLivingston/boomboom-plugin-quickstart-typescript/) (also available on [framagit](https://framagit.org/Livingston/boomboom-plugin-quickstart-typescript/)) instead of `boomboom-plugin-quickstart`.
Please read carefully the [README file](https://github.com/JohnXLivingston/boomboom-plugin-quickstart-typescript/blob/main/README.md), as there are some other differences with `boomboom-plugin-quickstart` (using SCSS instead of CSS, linting rules, ...).

If you don't want to use `boomboom-plugin-quickstart-typescript`, you can also manually add a dev dependency to __Boomboom__ types:

```
npm install --save-dev @boomboom/boomboom-types
```

This package exposes *server* definition files by default:
```ts
import { RegisterServerOptions } from '@boomboom/boomboom-types.js'
...
But it also exposes client types and various models used in __BoomBoom__:
```ts
import { Video } from '@boomboom/boomboom-types.js';
import { RegisterClientOptions } from '@boomboom/boomboom-types/client.js';

function register({ registerHook, boomboomHelpers }: RegisterClientOptions) {
  registerHook({
...
The key should be one of the locales defined in [i18n.ts](https://github.com/Chocobozzz/BoomBoom/blob/develop/packages/core-utils/src/i18n/i18n.ts).
...
You need to have a local BoomBoom instance with an administrator account.
...
Install BoomBoom CLI (can be installed on another computer/server than the BoomBoom instance):

```bash
npm install -g @boomboom/boomboom-cli
```

Register the BoomBoom instance via the CLI:

```sh
boomboom-cli auth add -u 'https://boomboom.example.com' -U 'root' --password 'test'
```

Then, you can install your local plugin/theme.
The `--path` option is the local path on the BoomBoom instance.
If the BoomBoom instance is running on another server/computer, you must copy your plugin directory there.

```sh
boomboom-cli plugins install --path /your/absolute/plugin-or-theme/path
```
...
and republish it on NPM. Remember that the BoomBoom index will take into account your new plugin/theme version after ~24 hours.

> If you need to force your plugin update on a specific __BoomBoom__ instance, you may update the latest available version manually:
> ```sql
...
you can deprecate it. The plugin index will automatically remove it preventing users to find/install it from the BoomBoom admin interface:

```bash
npm deprecate boomboom-plugin-xxx@"> 0.0.0" "explain here why you deprecate your plugin/theme"
```

## Plugin & Theme hooks/actions/helpers API

See the dedicated documentation: https://docs.joinboomboom.org/api/plugins


## Tips

### Compatibility with BoomBoom

Unfortunately, we don't have enough resources to provide hook compatibility between minor releases of BoomBoom (for example between `1.2.x` and `1.3.x`).
So please:
...
```
  * Don't try to require parent BoomBoom modules, only use `boomboomHelpers`. If you need another helper or a specific hook, please [create an issue](https://github.com/Chocobozzz/BoomBoom/issues/new/choose)
  * Don't use BoomBoom dependencies. Use your own :)

If your plugin is broken with a new BoomBoom release, update your code and the `boomboomEngine` field of your `package.json` field.
This way, older BoomBoom versions will still use your old plugin, and new BoomBoom versions will use your updated plugin.
...
You can take a look to "official" BoomBoom plugins if you want to take inspiration from them: https://framagit.org/framasoft/boomboom/official-plugins
