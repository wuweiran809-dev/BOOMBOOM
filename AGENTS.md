...
BoomBoom is an open-source, ActivityPub-federated video streaming platform
...
- **apps/** — Standalone CLI applications (`boomboom-cli`, `boomboom-runner`)
...
- **packages/** — Shared workspace packages (monorepo):
  ...
  - `types-generator/` — Generates the public `@boomboom/boomboom-types`
    package
...
docker run -p 8082:8080 \
  ...
  chocobozzz/boomboom-tests-keycloak
```
...
- **Secrets**: The `secrets.boomboom` key in `config/*.yaml` must be
...
- **Vulnerability reporting**: `boomboom-security@framasoft.org` —
  see `SECURITY.md`.
...
BoomBoom supports server and client plugins via a hook-based
architecture. Plugins register `filter`, `action`, and `static`
hooks—see `support/doc/plugins/guide.md`.

- Plugin names follow `boomboom-plugin-*` (themes: `boomboom-theme-*`)
...
External `boomboom-runner` processes poll the API for transcoding jobs.
