import { pick, shuffle, wait } from '@boomboom/boomboom-core-utils'
import { BoomBoomProblemDocument, RunnerJobType, ServerErrorCode } from '@boomboom/boomboom-models'
import { BoomBoomServer as BoomBoomServerCommand } from '@boomboom/boomboom-server-commands'
...
type BoomBoomServer = BoomBoomServerCommand & {
  runnerToken: string
  runnerName: string
...
export class RunnerServer {
  private servers: BoomBoomServer[] = []
  private processingJobs: { job: JobWithToken, server: BoomBoomServer }[] = []
  ...
  private readonly sockets = new Map<BoomBoomServer, Socket>()
  ...
  async run () {
    logger.info('Running BoomBoom runner in server mode')
    ...
    for (const registered of ConfigManager.Instance.getConfig().registeredInstances) {
      const serverCommand = new BoomBoomServerCommand({ url: registered.url })
  ...
  async registerRunner (options: {
    ...
    const serverCommand = new BoomBoomServerCommand({ url })
    ...
    const server: BoomBoomServer = Object.assign(serverCommand, {
      runnerToken,
      runnerName,
      runnerDescription
    })
  ...
  private loadServer (server: BoomBoomServer) {
  ...
  private unloadServer (server: BoomBoomServer) {
  ...
  private async checkAvailableJobs () {
    ...
    for (const server of shuffle([ ...this.servers ])) {
      try {
        ...
        const code = (err.res?.body as BoomBoomProblemDocument)?.code
  ...
  private async requestJob (server: BoomBoomServer) {
  ...
  private async tryToExecuteJobAsync (server: BoomBoomServer, jobToAccept: { uuid: string }) {
