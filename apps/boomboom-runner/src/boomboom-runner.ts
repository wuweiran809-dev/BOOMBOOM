...
import { RunnerJobType } from '@boomboom/boomboom-models'
...
const program = new Command()
  ...
  .option(
    '--id <id>',
    'Runner server id, so you can run multiple BoomBoom server runners with different configurations on the same machine',
    'default'
  )
...
program.command('server')
  .description('Run in server mode, to execute remote jobs of registered BoomBoom instances')
  .option(
    '--enable-job <type>',
  ...
  .action(async options => {
    try {
      ...
      logger.error(err, 'Cannot run BoomBoom runner as server mode')
      process.exit(-1)
    }
  })

program.command('register')
  .description('Register a new BoomBoom instance to process runner jobs')
  .requiredOption('--url <url>', 'BoomBoom instance URL', parseUrl)
  .requiredOption('--registration-token <token>', 'Runner registration token (can be found in BoomBoom instance administration')
  ...
  .action(async options => {
    try {
      ...
      console.error('Cannot register this BoomBoom runner.')
      console.error(err)
      process.exit(-1)
    }
  })
...
program.command('unregister')
  .description('Unregister the runner from BoomBoom instance')
  .requiredOption('--url <url>', 'BoomBoom instance URL', parseUrl)
  .requiredOption('--runner-name <name>', 'Runner name')
  .action(async options => {
    try {
      ...
      console.error('Cannot unregister this BoomBoom runner.')
      console.error(err)
      process.exit(-1)
    }
  })
...
program.command('list-registered')
  .description('List registered BoomBoom instances')
  .action(async () => {
    try {
      await listRegistered()
    } catch (err) {
      console.error('Cannot list registered BoomBoom instances.')
      console.error(err)
      process.exit(-1)
    }
  })
