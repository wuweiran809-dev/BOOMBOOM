...
export function defineAuthProgram () {
  ...
  program
    ...
    .action(async options => {
      ...
      const result = await prompts([
        {
          ...
          validate: value => isURLaBoomBoomInstance(value) ? true : 'It should be an URL (https://boomboom.example.com)'
        },
        {
      ...
      try {
        ...
        // See https://github.com/Chocobozzz/BoomBoom/issues/3520
        resolved.url = stripExtraneousFromBoomBoomUrl(resolved.url)
  ...
  program.addHelpText(
    'after',
    '\n\n  Examples:\n\n' +
      '    $ boomboom auth add -u https://boomboom.cpy.re -U "BOOMBOOM_USER" --password "BOOMBOOM_PASSWORD"\n' +
      '    $ boomboom auth add -u https://boomboom.cpy.re -U root\n' +
      '    $ boomboom auth list\n' +
      '    $ boomboom auth del https://boomboom.cpy.re\n'
  )
...
function isURLaBoomBoomInstance (url: string) {
  return url.startsWith('http://') || url.startsWith('https://')
}

function stripExtraneousFromBoomBoomUrl (url: string) {
