...
import { VideoCommentPolicy, VideoPrivacy, VideoPrivacyType } from '@boomboom/boomboom-models'
import { BoomBoomServer, VideoEdit } from '@boomboom/boomboom-server-commands'
...
const program = new Command('upload')
  .description('Upload a video on a BoomBoom instance')
  .alias('up')
...
async function run (options: ReturnType<typeof program.opts>) {
  ...
  try {
    ...
    if (message.includes('413')) {
      console.error('Aborted: user quota is exceeded or video file is too big for this BoomBoom instance.')
    } else {
...
async function buildVideoAttributesFromCommander (server: BoomBoomServer, options: ReturnType<typeof program.opts>) {
  const videoAttributes: VideoEdit = {
    ...
    commentsPolicy: options.commentsEnabled === false
    ...
    downloadEnabled: options.downloadEnabled === false
      ? false
      : undefined,
    waitTranscoding: options.waitTranscoding === false
      ? false
      : undefined
  }
