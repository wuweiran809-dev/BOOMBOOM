...
import { UserRole } from '@boomboom/boomboom-models'
import { getAppNumber, isTestInstance, root } from '@boomboom/boomboom-node-utils'
...
async function getAdminTokenOrDie (server: BoomBoomServer, username: string, password: string) {
...
function buildServer (url: string) {
  return new BoomBoomServer({ url })
}

async function assignToken (server: BoomBoomServer, username: string, password: string) {
